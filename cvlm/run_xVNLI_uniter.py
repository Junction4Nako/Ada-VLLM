...
class xVNLIDataset(Dataset):
        ...
        for line in open(caption_file, 'r'):
            ...
            if 'language' in info:
                if info['language'] not in args.valid_langs:
                    continue
            if int(info['Flikr30kID']) not in img_feat_file:
                continue
            self.data.append([info['pairID'], info['sentence2'], info['gold_label'], int(info['Flikr30kID'])])
    ...
    def get_image_feat(self, image_id):
        tmp_feat = self.img_feat[image_id]
        num_bbs = tmp_feat.shape[0]
        dense_feat = tmp_feat[:, :2048]
        bb_feat = tmp_feat[:, 2048:]
...
def main():
    ...
    img_feat_file = torch.load(uniter_config['image_root'])
    ...
    if args.do_train:
        train_dataset = xVNLIDataset(caption_file=args.data_file_train, tokenizer=tokenizer, args=args, img_feat_file=img_feat_file, is_train=True)
        if args.evaluate_during_training:
            if isinstance(args.data_file_val, dict):
                val_dataset = {k: xVNLIDataset(caption_file=v, tokenizer=tokenizer, args=args, img_feat_file=img_feat_file, is_train=False) for k,v in args.data_file_val.items()}
            else:
                val_dataset = xVNLIDataset(caption_file=args.data_file_val, tokenizer=tokenizer, args=args, img_feat_file=img_feat_file, is_train=False)
        else:
            val_dataset = None
    ...
    if args.do_test or args.do_eval:
        if args.eval_all_checkpoints:
            ...
            # args = restore_training_settings(args)
            if isinstance(args.data_file_val, dict):
                test_dataset = {k: xVNLIDataset(caption_file=v, img_feat_file=img_feat_file, tokenizer=tokenizer, args=args, is_train=False) for k,v in args.data_file_val.items()}
            else:
                test_dataset = xVNLIDataset(caption_file=args.data_file_val, img_feat_file=val_transform, tokenizer=tokenizer, args=args, is_train=False)
            checkpoint = args.eval_model_dir
            assert op.isdir(checkpoint)
            logger.info("Evaluate the following checkpoint: %s", checkpoint)

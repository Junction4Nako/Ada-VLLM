...
from albef.modeling.model_vqa import ALBEF_CLS, ALBEF_GEN
...
class GQADataset(Dataset):
    ...
    def __init__(self, caption_file, img_transform, tokenizer, args, is_train=True):
        ...
        if '*' in caption_file:
            # multiple files
            prefix, suffix = os.path.split(caption_file)
            tmp_path = Path(prefix)
            for fn in tmp_path.glob(suffix):
                tmp_split = json.load(open(fn, 'r'))
                if isinstance(tmp, dict):
                    for k,v in tmp_split.items():
                        self.data.append([k, v['question'], v['answer'], v['imageId']])
                elif isinstance(tmp, list):
                    for item in tmp:
                        if isinstance(item['label'], dict):
                            tmp_ans = list(item['label'].keys())[0]
                        else:
                            tmp_ans = item['label']
                        self.data.append([item['question_id'], item['sent'], tmp_ans, item['img_id']])
                else:
                    raise NotImplementedError
        else:
            tmp = json.load(open(caption_file, 'r'))
            if isinstance(tmp, dict):
        ...
        if args.model_type == 'classification':
            assert args.ans2label_map is not None, 'classfication model needs the answer2label mapping file!'
            if args.ans2label_map.endswith('json'):
                self.ans2label = json.load(open(args.ans2label_map, 'r'))
            elif args.ans2label_map.endswith('pkl'):
                self.ans2label = pickle.load(open(args.ans2label_map, 'rb'))
            self.label2ans = {v:k for k,v in self.ans2label.items()}
            global ood_index
            ood_index = len(self.label2ans)
            self.label2ans[ood_index] = 'none'
            logger.info('avoiding the ood index {}'.format(ood_index))

        if args.train_by_image and is_train:
            # consider images as samples
            self.train_by_image = True
            self.image_to_questions = defaultdict(list)
            for item in tqdm(self.data, desc='generating the image mapping'):
                self.image_to_questions[item[3]].append(item[:3])
            self.image_index = list(self.image_to_questions.keys())
        else:
            self.train_by_image = False
            self.image_to_questions = None
    ...
    def __getitem__(self, index):
        if self.train_by_image:
            image_id = self.image_index[index]
            question_id, question, answer = random.choice(self.image_to_questions[image_id]) # only sample one question for each image
        else:
            question_id, question, answer, image_id = self.data[index]
        feature = self.get_image(image_id)
        if self.model_type == 'classification':
            answer_id = self.ans2label[answer] if answer in self.ans2label else len(self.ans2label) # the last index is for "none of the answers"
        # print([i.shape for i in example])
            return index, (feature, question, answer_id)
        else:
            return index, (feature, question, answer)

    def get_question_id(self, index):
        return self.data[index][0]

    def get_raw_answer(self, index):
...
def compute_score_with_logits(logits, labels):
    if logits.shape[1] > 1:
        ...
        if ood_index == -1:
            ...
            # avoid ood judgement
            scores = (logits == labels) & (logits != ood_index)
    else:
...
def train(args, train_dataset, val_dataset, model, tokenizer):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, (_, batch) in enumerate(train_dataloader):
            ...
            if args.model_type == 'generation':
                answer = tokenizer(list(answer), padding='longest', max_length=args.max_seq_length, return_tensors="pt").to(args.device)
            else:
        ...
        if args.evaluate_during_training:
            ...
            if isinstance(val_dataset, dict):
                ...
                for k,v in val_dataset.items():
                    logger.info('evaluate the language {}'.format(k))
                    tmp_result, tmp_score = evaluate(args, model, v, tokenizer)
                    current_score += tmp_score
            else:
                logger.info('evaluate the language {}'.format(args.test_lang))
                eval_result, eval_score = evaluate(args, model, val_dataset, tokenizer)
...
def test(args, model, eval_dataset, tokenizer):
    ...
    label2ans = list(eval_dataset.values())[0].label2ans if isinstance(eval_dataset, dict) else eval_dataset.label2ans
    eval_loss = 0.0
    nb_eval_steps = 0
    preds = None
    out_label_ids = None
    ...
    #for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            # update results_dict
            results_dict.update(
                {eval_dataset.get_question_id(qa_ind): (eval_dataset.get_raw_answer(qa_ind),label2ans[p]) for qa_ind, p in
    ...
    if args.compute_bleu:
        ...
        for k,v in results_dict.items():
            gt_ans, pred_ans = v
            if pred_ans == 'none':
                tmp_bleu.append([0.0]*5)
            else:
                gt_ans = [t for t in gt_ans]
                pred_ans = [t for t in pred_ans]
                ttt = []
                for i in range(4):
                    w = [0,]*4
                    w[i] = 1
                    ttt.append(sentence_bleu([gt_ans], pred_ans, weights=tuple(w)))
                ttt.append(sentence_bleu([gt_ans], pred_ans))
        for i in range(5):
            kkk = [item[i] for item in tmp_bleu]
            score_i = sum(kkk)/len(kkk)
            logger.info('BLEU@{}:{}'.format(i+1, score_i))
...
def main():
    ...
    parser.add_argument('--data_debug', action='store_true', help='whether to use part of the data to debug')
    parser.add_argument('--image_dir_format', type=str, default='remote', help='2 setting for loading images')
    ...
    if albef_config['model_type'] == 'classification': 
        model_class = ALBEF_CLS
    elif albef_config['model_type'] == 'generation':
        model_class = ALBEF_GEN
    else:
        raise NotImplementedError
    ...
    if args.model_name_or_path is not None:
        ...
        if args.do_train:
            for key in list(state_dict.keys()):
                ...
                if args.model_type == 'classification':
                    if 'text_encoder.cls.predictions' in key:
                        encoder_key = key.replace('text_encoder.cls.predictions', 'qa_head')
                        state_dict[encoder_key] = state_dict[key]
                        del state_dict[key]
                    else:
            ...
            if args.model_type == 'classification':
                if args.ans2label_map.endswith('json'):
                    ans2label = json.load(open(args.ans2label_map, 'r'))
                elif args.ans2label_map.endswith('pkl'):
                    ans2label = pickle.load(open(args.ans2label_map, 'rb'))
                # global ood_index
                # ood_index = len(ans2label)
                if args.original_ckpt is not None and args.original_tokenizer is not None:
                    ori_tokenizer = AutoTokenizer.from_pretrained(args.original_tokenizer)
                    ori_sd = torch.load(args.original_ckpt, map_location='cpu')
                    ori_sd = ori_sd['model'] if 'model' in ori_sd else ori_sd
                    new_weight, new_bias = reinit_qa_head(tokenizer=ori_tokenizer, ans2label=ans2label, weight=ori_sd['text_encoder.cls.predictions.decoder.weight'], bias=ori_sd['text_encoder.cls.predictions.bias'])
                    del ori_sd
                    del ori_tokenizer
                else:
                    new_weight, new_bias = reinit_qa_head(tokenizer=tokenizer, ans2label=ans2label, weight=state_dict['qa_head.decoder.weight'], bias=state_dict['qa_head.bias'])
                # print(new_bias)
                state_dict['qa_head.decoder.weight'] = new_weight
                state_dict['qa_head.bias'] = new_bias
                state_dict['qa_head.decoder.bias'] = new_bias
    ...
    if args.do_train:
        train_dataset = GQADataset(caption_file=args.data_file_train, img_transform=train_transform, tokenizer=tokenizer, args=args, is_train=True)
        if args.evaluate_during_training:
            if isinstance(args.data_file_val, dict):
                val_dataset = {k:GQADataset(caption_file=v, img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False) for k,v in args.data_file_val.items()}
            else:
                val_dataset = GQADataset(caption_file=args.data_file_val, img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False)
        else:
            val_dataset = None
    ...
    if args.do_test or args.do_eval:
        if args.eval_all_checkpoints:
            ...
            test_dataset = GQADataset(caption_file=args.data_file_val, img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False)
            ...
            # args = restore_training_settings(args)
            if isinstance(args.data_file_val, dict):
                test_dataset = {k:GQADataset(caption_file=v, img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False) for k,v in args.data_file_val.items()}
            else:
                test_dataset = GQADataset(caption_file=args.data_file_val, img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False)
            checkpoint = args.eval_model_dir
            assert op.isdir(checkpoint)
            ...
            if isinstance(test_dataset, dict):
                for k,v in test_dataset.items():
                    logger.info('evaluate on language {}'.format(k))
                    tmp_res, tmp_score = test(args, model, v, tokenizer=tokenizer)
            else:
                logger.info('evaluate on language {}'.format(args.test_lang))
                test_res, test_score = test(args, model, test_dataset, tokenizer=tokenizer)

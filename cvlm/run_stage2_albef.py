...
from oscar.oscar_datasets_ml_img.build import make_data_loader, make_data_loader_ds, make_data_loader_od_ds
from oscar.parallel_txt_datasets.build import make_data_loader_ds as make_txt_data_loader
...
def main():
    ...
    parser.add_argument('--trans_dataset_file', type=str, default=None, help='the dataset file for translated mm pairs')
    parser.add_argument('--img_format', type=str, default='raw', help='the image storage format')
    ...
    logger.info('training with languages {}'.format(','.join(args.languages)))
    ...
    if args.model_name_or_path is not None:
        ...
        if model.text_encoder.bert.embeddings.word_embeddings.weight.shape[0] != state_dict['text_encoder.bert.embeddings.word_embeddings.weight'].shape[0]:
            ...
            logger.info('found the word embedding size is not aligned, re-initializing!')
    ...
    # image-text pair
    train_dataloaders = make_data_loader_od_ds(
    ...
    for step, (batch, batch_extra) in enumerate(zip(train_dataloader, train_dataloader_extra), start_iter):
        ...
        def forward_backward(image_features, text_input, loss_weight=1.0):
            if args.deepspeed:
                ...
                outputs = model(image=image_features, text=text_input)
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            metrics_to_log = {
                'time_info': {'compute': batch_time/args.gradient_accumulation_steps, 'data': data_time,
                              ...
                              'compute2': compute_time2, 'backward': backward_time},
                'batch_metrics': {'loss': loss1+loss2, 'mlm_loss':mlm_loss1+mlm_loss2, 'ita_loss':ita_loss1+ita_loss2, 

...
from albef.modeling_e.model_vqa import ALBEF_CLS, ALBEF_GEN
        ...
        if '*' in caption_file:
            ...
            for fn in tmp_path.glob(suffix):
                ...
                if isinstance(tmp, dict):
                    ...
                    for item in tmp:
                        self.data.append([item['question_id'], item['sent'], list(item['label'].keys())[0], item['img_id']])
                else:
...
def compute_score_with_logits(logits, labels):
    if logits.shape[1] > 1:
        ood_index = logits.shape[1]
        logits = torch.max(logits, 1)[1].data # argmax
        scores = (logits == labels) & (labels != ood_index)  
    else:
...
def main():
    ...
    if args.eval_split == 'val':
        if args.test_lang == 'all':
            ...
            args.data_file_val = albef_config['val_file'][args.test_lang]

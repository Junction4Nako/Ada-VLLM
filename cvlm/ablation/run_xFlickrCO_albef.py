...
from albef.modeling_e.model_retrieval import ALBEF
...
from oscar.parallel_txt_datasets.build import make_data_loader_ds as make_parallel_txt
...
class RetrievalDataset(Dataset):
    ...
    def __init__(self, caption_file, img_transform, tokenizer, args, is_train=True, coarse_cap_index=None, coarse_img_index=None, simplified=False):
        """
        ...
        if caption_file.endswith('.pth') or caption_file.endswith('.pt'):
            ...
            for line in open(caption_file, 'r'):
                ...
                if simplified:
                    self.captions[os.path.splitext(info['img_path'])[0]] = info['sentences'][:1]
                else:
                    self.captions[os.path.splitext(info['img_path'])[0]] = info['sentences']
        else:
        ...
        for img_index, (k,v) in tqdm(enumerate(self.captions.items()), desc='creating the caption to image index'):
    ...
    def get_image(self, image_full_id):
        ...
        if image_id.endswith('.jpg'):
            img_path = os.path.join(self.image_dir[root_part], image_id)
        else:
            img_path = os.path.join(self.image_dir[root_part], '{}.jpg'.format(image_id))
...
def train(args, train_dataset, val_dataset, model, tokenizer):
    ...
    args.max_iters = t_total
    if args.add_txt:
        txt_loader = iter(make_parallel_txt(args, tokenizer, is_distributed=False)[0])
    else:
        txt_loader = None
    ...
    for epoch in range(int(args.num_train_epochs)):
        for step, (_, batch) in enumerate(train_dataloader):
            ...
            if txt_loader is not None:
                txt_batch = next(txt_loader)
                src_txts = txt_batch['source_text']
                tar_txts = txt_batch['target_text']
                src_text_input = tokenizer(src_txts, padding='longest', truncation=True, max_length=args.max_seq_length_txt, return_tensors="pt").to(args.device)
                tar_text_input = tokenizer(tar_txts, padding='longest', truncation=True, max_length=args.max_seq_length_txt, return_tensors="pt").to(args.device)
                txt_outputs = model(None, src_text_input.input_ids, src_text_input.attention_mask, tar_text_input.input_ids, tar_text_input.attention_mask, forward_mod='txt')
                t2t_loss = txt_outputs[0]
                if args.n_gpu > 1: 
                    t2t_loss = t2t_loss.mean() # mean() to average on multi-gpu parallel training
                if args.gradient_accumulation_steps > 1:
                    t2t_loss = t2t_loss / args.gradient_accumulation_steps
                t2t_loss.backward()
                global_t2t_loss += t2t_loss.item()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.logging_steps == 0:
                    tmp_info ="Epoch: {}, global_step: {}, lr: {:.6f}, loss: {:.4f} ({:.4f}), " \
                    ...
                    if txt_loader is not None:
                        tmp_info = '{}, t2t loss: {:.4f} ({:.4f})'.format(tmp_info, t2t_loss.item(), global_t2t_loss/global_step)
                    ...
                    if args.evaluate_during_training: 
                        ...
                        if isinstance(val_dataset, dict):
                            ...
                            for k in val_dataset:
                                logger.info('evaluating on language {}'.format(k))
                                logger.info('coarse inference:')
                                coarse_sim = test_coarse(args, model, val_dataset[k], tokenizer)
                                eval_result, caption_index, image_index = evaluate_coarse(val_dataset[k], coarse_sim)
                                # caption index and image index
                                eval_i2t_result, _ = test_fine_i2t(args, model, val_dataset[k], caption_index=caption_index, tokenizer=tokenizer)
                                eval_t2i_result = test_fine_t2i(args, model, val_dataset[k], image_index=image_index, tokenizer=tokenizer)
                                logger.info('fine inference:')
                                # print(eval_i2t_result, eval_t2i_result)
                                eval_result = evaluate_fine(eval_i2t_result, eval_t2i_result)

                                TR[k] = eval_result['i2t_retrieval']
                                IR[k] = eval_result['t2i_retrieval']
                                rank_accs = eval_result['i2t_retrieval']
                                if args.save_metric == 'r1':
                                    current_score += rank_accs['R@1'] + eval_result['t2i_retrieval']['R@1']
                            ...
                            epoch_log = {'epoch': epoch, 'global_step': global_step, 'current_{}'.format(args.save_metric): current_score,
                                        'TR': TR, 'IR': IR, 'best_{}'.format(args.save_metric):best_score}
                            ...
                            #     best_score = rank_accs['R@1']
                            epoch_log = {'epoch': epoch, 'global_step': global_step, 'current_{}'.format(args.save_metric):current_score,
                                        'TR': rank_accs, 'IR': eval_result['t2i_retrieval'],'best_{}'.format(args.save_metric):best_score}
                            log_json.append(epoch_log)
                            with open(args.output_dir + '/eval_logs.json', 'w') as fp:
                                json.dump(log_json, fp) 
            if args.time_debug:
...
def main():
    ...
    parser.add_argument('--test_lang', type=str, default=None, help='the test language')
    parser.add_argument('--add_txt', action='store_true', help='using parallel txt to help training')
    ...
    parser.add_argument('--max_seq_length_txt', type=int, default=50)
    parser.add_argument('--txt_dataformat', type=str, default=None, help='the dataset format')
    ...
    if args.test_lang == 'all':
        args.valid_langs = list(args.data_file_val.keys())
    else:
        args.valid_langs = [args.test_lang]
    args.valid_langs.append('en')
    ...
    if args.do_train:
        ...
        if args.evaluate_during_training:
            if isinstance(args.data_file_val, dict):
                val_dataset = {k: RetrievalDataset(caption_file=v, img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False, simplified=k!='en') for k,v in args.data_file_val.items()}
            else:
                sim = args.test_lang != 'en'
                val_dataset = RetrievalDataset(caption_file=args.data_file_val, img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False, simplified=sim)
        else:
            val_dataset = None
        ...
        if args.eval_all_checkpoints:
            ...
            if isinstance(test_dataset, dict):
                for k in test_dataset:
                    logger.info('evaluating on language {}'.format(k))
                    coarse_sim = test_coarse(args, model, test_dataset[k], tokenizer=tokenizer)
                    eval_result, caption_index, image_index = evaluate_coarse(test_dataset[k], coarse_sim)
                    # caption index and image index
                    eval_i2t_result, _ = test_fine_i2t(args, model, test_dataset[k], caption_index=caption_index, tokenizer=tokenizer)
                    eval_t2i_result = test_fine_t2i(args, model, test_dataset[k], image_index=image_index, tokenizer=tokenizer)
                    print('fine inference:')
                    # print(eval_i2t_result, eval_t2i_result)
                    if args.do_eval:
                        eval_result = evaluate_fine(eval_i2t_result, eval_t2i_result)
                ...
                logger.info('evaluating on language {}'.format(args.test_lang))
                coarse_sim = test_coarse(args, model, test_dataset, tokenizer=tokenizer)
                eval_result, caption_index, image_index = evaluate_coarse(test_dataset, coarse_sim)
                # caption index and image index
                eval_i2t_result, _ = test_fine_i2t(args, model, test_dataset, caption_index=caption_index, tokenizer=tokenizer)

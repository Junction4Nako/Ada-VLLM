...
class xNLIDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return index, (cond_id, question, label_id)
...
def train(args, train_dataset, val_dataset, model, tokenizer):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, (_, batch) in enumerate(train_dataloader):
            ...
            outputs = model(text.input_ids, text.attention_mask, condition.input_ids, condition.attention_mask, answer)
            loss, logits = outputs
            # print('return', logits.shape)
            if args.n_gpu > 1: 
...
def main():
    ...
    # dataset loading
    args.valid_langs = albef_config['valid_languages']
    ...
    if albef_config['model_type'] == 'classification': 
        model_class = ALBEFforTxtClassification
    else:
        raise NotImplementedError
    ...
    if args.do_test or args.do_eval:
        if args.eval_all_checkpoints:
            ...
            test_dataset = xNLIDataset(caption_file=args.data_file_val, img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False)

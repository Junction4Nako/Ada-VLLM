...
from oscar.modeling.modeling_vlbert_pretrain import BiBertImgForCVLM
from transformers_past.pytorch_transformers import (WEIGHTS_NAME, BertConfig,
                                  BertTokenizer)

from oscar.oscar_datasets_ml_hdfs.build import make_data_loader, make_data_loader_ds
...
MODEL_CLASSES = {
    'bert': (BertConfig, BiBertImgForCVLM, BertTokenizer),
}
...
def main():
    ...
    # print(model)
    single_modal_keys = ['vis_encoder', 'txt_encoder', 'embeddings', 'txt_proj', 'vis_proj', 'phrase_mlm', 'half_mlm', \
        'LayerNorm', 'img_embedding', 'logit_scale']
    mul_modal_keys = ['mul_encoder', 'bert.pooler', 'cls', 'qa_seq_relation']
    if args.uni_lr is None:
        ...
        # to make 2 stage with different learning rate
        uni_no_decay = []
        mul_no_decay = []
        uni_decay = []
        mul_decay = []
        ...
        for n, p in param_optimizer:
            ...
            if any(nd in n for nd in no_decay):
                # no decay
                if any(u in n for u in mul_modal_keys):
                    # mul-modal part
                    mul_no_decay.append(p)
                    mul_keys.append(n)
                    
                else:
                    # uni-modal part
                    uni_no_decay.append(p)
                    uni_keys.append(n)
            else:
                # with decay 
                if any(u in n for u in mul_modal_keys):
        ...
        # return None
        logger.info("uni model keys with lr {}:".format(args.uni_lr) + ' '.join(uni_keys))
        logger.info("multi model keys with lr {}:".format(args.learning_rate) + ' '.join(mul_keys))
        optimizer_grouped_parameters = [
            # uni-modal with weight decay
            {'params': uni_decay,
            'weight_decay': 0.01, 'lr': args.uni_lr},
            # multi-modal with weight decay
            {'params': mul_decay,
            'weight_decay': 0.01, 'lr': args.learning_rate},
            {'params': uni_no_decay,
            'weight_decay': 0.0, 'lr': args.uni_lr},
            {'params': mul_no_decay,
    ...
    for step, (batch, batch_extra) in enumerate(zip(train_dataloader, train_dataloader_extra), start_iter):
        ...
        def data_process(mini_batch):
            ...
            return images, input_ids_a, input_mask_a, segment_ids_a, lm_label_ids_a, input_ids_b, input_mask_b, segment_ids_b, lm_label_ids_b

        images1, input_ids_a1, input_mask_a1, segment_ids_a1, lm_label_ids_a1, input_ids_b1, input_mask_b1, segment_ids_b1, lm_label_ids_b1  = data_process(batch)
        if not args.phrase_mask:
            phrase_mask_label1 = None

        data_time = time.time() - end

        def forward_backward(image_features, input_ids_a, input_mask_a, segment_ids_a,lm_label_ids_a, input_ids_b, input_mask_b, 
                            segment_ids_b, lm_label_ids_b, qa_is_next=None, phrase_index=None, image_index=None, phrase_mask_label=None, loss_weight=1.0):
            # feature as input
            ...
            if args.deepspeed:
                ...
                outputs, debug_output = model_engine(input_ids_a=input_ids_a, token_type_ids_a=segment_ids_a, attention_mask_a=input_mask_a,
                                masked_lm_labels_a=lm_label_ids_a, input_ids_b=input_ids_b, img_feats=image_features, use_hungarian=args.use_hungarian,
                                token_type_ids_b=segment_ids_b, attention_mask_b=input_mask_b, masked_lm_labels_b=lm_label_ids_b, phrase_mask_label=phrase_mask_label,
                                phrase_index=phrase_index, img_index=image_index, max_tag_length=args.max_tag_length, phrase_mod=args.wra_mod, qa_is_next=qa_is_next, phrase_layer=args.wra_layer)
            else:
            ...
            if args.phrase_mask:
                total_loss, vis_mlm_loss, retrieval_loss, masked_lm_loss, next_sentence_loss, masked_phrase_loss, qa_rel_loss, wra_loss = outputs
            else:
            ...
            if args.phrase_mask:
                masked_phrase_loss = loss_weight * masked_phrase_loss.item()
            else:
                masked_phrase_loss = 0
            ...
            return loss.item(), input_ids_a.size(0), wra_loss, vis_mlm_loss, retrieval_loss, mlm_loss, relation_loss, qa_rel_loss, masked_phrase_loss
        ...
        loss1 = 0.0
        nb_tr_example1 = 1
        wra_loss1, vis_mlm_loss1, retrieval_loss1 = 0.0, 0.0, 0.0
        mlm_loss1, relation_loss1, qa_rel_loss1, phrase_mask_loss1 = 0.0, 0.0, 0.0, 0.0
        # return None
        tr_loss += loss1
        ...
        wra_loss2, mlm_loss2 = 0.0, 0.0
        phrase_mask_loss2 = 0.0
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            metrics_to_log = {
                ...
                'CLIP_loss': retrieval_loss1+retrieval_loss2, 'rel_loss': relation_loss1+relation_loss2, 'qa_rel_loss': qa_rel_loss1+qa_rel_loss2, 'phrase_mask_loss': phrase_mask_loss1+phrase_mask_loss2}
            }

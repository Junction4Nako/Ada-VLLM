...
class VQADataset(Dataset):
    """ Text only Dataset"""
    # GQA dataset
    def __init__(self, caption_file, tokenizer, args, img_feat_file, is_train=True):
        """
        ...
        img_db = img_feat_file['img_db']
        self.num2bb = json.load(open(img_feat_file['num2bb'], 'r'))
        self.env = lmdb.open(img_db, readonly=True, create=False)
    ...
    def __getitem__(self, index):
        ...
        label_id = self.ans2label[label] if label in self.ans2label else len(self.ans2label)
        num_bbs, dense_feat, bb_feat = self.get_image_feat(image_id)
        tmp_tokens = self.tokenizer.tokenize(question)
        if len(tmp_tokens) > self.max_seq_len-2:
            tmp_tokens = tmp_tokens[:(self.max_seq_len-2)]
            pad_token_len = 0
        else:
            pad_token_len = self.max_seq_len-2-len(tmp_tokens)
        question_token = [self.tokenizer.cls_token] + tmp_tokens + [self.tokenizer.sep_token]
        valid_token_len = len(question_token)
        question_token = question_token + [self.tokenizer.pad_token]*pad_token_len
        ...
        input_ids = self.tokenizer.convert_tokens_to_ids(question_token)
        # input_ids = [0]*len(input_ids)
        attn_masks = [1]*(valid_token_len+num_bbs)+[0]*(self.max_img_seq_len+self.max_seq_len-(valid_token_len+num_bbs))
        attn_masks = torch.tensor(attn_masks, dtype=torch.long)
        input_ids = torch.tensor(input_ids, dtype=torch.long)
        out_size = self.max_img_seq_len+self.max_seq_len
        gather_index = torch.arange(0, out_size, dtype=torch.long)
        gather_index.data[valid_token_len:valid_token_len+num_bbs] = torch.arange(self.max_seq_len, self.max_seq_len+num_bbs,
                                                       dtype=torch.long).data
        # print(gather_index)
        return index, (input_ids, dense_feat, bb_feat, attn_masks, gather_index, label_id)
    ...
    def get_image_feat(self, image_id):
        if 'COCO' in image_id:
            valid_key = image_id.split('/')[1].split('.')[0].lower()+'.npz'
            tmp_info = self.txn.get(valid_key.encode('utf-8'))
        else:
            valid_key = 'vg_%012d.npz' % int(image_id)
            tmp_info = self.txn.get(valid_key.encode('utf-8'))
        tmp_info = msgpack.loads(tmp_info, raw=False)

        num_bbs = self.num2bb[valid_key]
        dense_feat = np.frombuffer(tmp_info['features'][b'data'], dtype=np.float16).reshape(num_bbs, 2048)
        dense_feat = torch.from_numpy(dense_feat.copy()).float()
        bb_feat = np.frombuffer(tmp_info['norm_bb'][b'data'], dtype=np.float16).reshape(num_bbs, 6)
        bb_feat = torch.from_numpy(bb_feat.copy()).float()
        bb_feat = torch.cat([bb_feat, bb_feat[:, 4:5]*bb_feat[:, 5:]], dim=1)
        if num_bbs > self.max_img_seq_len:
            num_bbs = self.max_img_seq_len
            dense_feat = dense_feat[:self.max_img_seq_len, :]
            bb_feat = bb_feat[:self.max_img_seq_len, :]
        else:
            dense_feat = torch.cat([dense_feat, torch.zeros([self.max_img_seq_len-num_bbs, dense_feat.shape[1]], dtype=dense_feat.dtype)], dim=0)
            bb_feat = torch.cat([bb_feat, torch.zeros([self.max_img_seq_len-num_bbs, bb_feat.shape[1]], dtype=bb_feat.dtype)], dim=0)

        return num_bbs, dense_feat, bb_feat
...
def train(args, train_dataset, val_dataset, model, tokenizer):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, (_, batch) in enumerate(train_dataloader):
            model.train()
            # (input_ids, dense_feat, bb_feat, attn_masks, gather_index, label_id)
            batch = {'input_ids': batch[0],
             'img_feat': batch[1],
             'img_pos_feat': batch[2],
             'attention_mask': batch[3],
             'gather_index': batch[4],
             'labels': batch[5]}
...
def test(args, model, eval_dataset, tokenizer):
    ...
    label2ans = eval_dataset.label2ans #  {v:k for k,v in VNLI_labelmap.items()}
    eval_loss = 0.0
    nb_eval_steps = 0
    preds = None
    out_label_ids = None
    ...
    #for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            # update results_dict
            results_dict.update(
                {qa_ind: (eval_dataset.get_raw_answer(qa_ind), label2ans[p]) for qa_ind, p in
...
def main():
    ...
    img_feat_file = uniter_config['image_root'][args.image_dir_format]
    if 'train' in img_feat_file:
        train_img_feat_file = img_feat_file['train']
        val_img_feat_file = img_feat_file['val']
    else:
        train_img_feat_file = img_feat_file
        val_img_feat_file = img_feat_file
    if args.eval_split == 'val':
        if args.test_lang == 'all':
            args.data_file_val = uniter_config['val_file']
        else:
            args.data_file_val = uniter_config['val_file'][args.text_lang]
    elif args.eval_split == 'test':
        if args.test_lang == 'all':
            args.data_file_val = uniter_config['test_file']
        else:
            args.data_file_val = uniter_config['test_file'][args.test_lang]
    ...
    if args.do_train:
        ...
        model = model_class.from_pretrained(bert_config, state_dict=checkpoint, num_cls=uniter_config['answer_num'], img_dim=2048)
    ...
    if args.do_train:
        train_dataset = VQADataset(caption_file=args.data_file_train, tokenizer=tokenizer, args=args, img_feat_file=train_img_feat_file, is_train=True)
        if args.evaluate_during_training:
            if isinstance(args.data_file_val, dict):
                val_dataset = {k: VQADataset(caption_file=v, tokenizer=tokenizer, args=args, img_feat_file=val_img_feat_file, is_train=False) for k,v in args.data_file_val.items()}
            else:
                val_dataset = VQADataset(caption_file=args.data_file_val, tokenizer=tokenizer, args=args, img_feat_file=val_img_feat_file, is_train=False)
        else:
            val_dataset = None
    ...
    if args.do_test or args.do_eval:
        if args.eval_all_checkpoints:
            ...
            # args = restore_training_settings(args)
            if isinstance(args.data_file_val, dict):
                test_dataset = {k: VQADataset(caption_file=v, img_feat_file=val_img_feat_file, tokenizer=tokenizer, args=args, is_train=False) for k,v in args.data_file_val.items()}
            else:
                test_dataset = VQADataset(caption_file=args.data_file_val, img_feat_file=val_img_feat_file, tokenizer=tokenizer, args=args, is_train=False)
            checkpoint = args.eval_model_dir
            assert op.isdir(checkpoint)
            logger.info("Evaluate the following checkpoint: %s", checkpoint)

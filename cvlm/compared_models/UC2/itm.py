...
class VLXLMRForImageTextRetrieval(VLXLMRPreTrainedModel):
...
class VLXLMRForImageTextClassification(VLXLMRPreTrainedModel):
    """ Finetune UNITER for image text retrieval
    """
    def __init__(self, config, img_dim, num_cls=1):
        super().__init__(config)
        self.roberta = VLXLMRModel(config, img_dim)
        self.cls_output = nn.Linear(config.hidden_size, num_cls)
        self.apply(self.init_weights)

    def forward(self, input_ids, attention_mask, gather_index=None, img_feat=None, img_pos_feat=None, labels=None, token_type_ids=None, position_ids=None):
        position_ids=None
        
        # print('in model', input_ids.shape)
        sequence_output = self.roberta(input_ids, position_ids,
                                    img_feat, img_pos_feat,
                                    attention_mask, gather_index,
                                    ...
                                    output_all_encoded_layers=False)
        pooled_output = self.roberta.pooler(sequence_output)
        cls_scores = self.cls_output(pooled_output)
        # print(cls_scores.shape)

        if labels is not None:
            ce_loss = nn.CrossEntropyLoss()
            rank_loss = ce_loss(cls_scores, labels)
            return rank_loss, cls_scores
        else:
            return cls_scores
#######################################################
class UniterForImageTextRetrieval(UniterPreTrainedModel):
    ...
    def __init__(self, config, img_dim, margin=0.2):
        ...
        self.itm_output = nn.Linear(config.hidden_size, 2)
        #self.itm_output = nn.Linear(config.hidden_size, 1)
        self.rank_output = nn.Linear(config.hidden_size, 1)
        self.margin = margin
        self.apply(self.init_weights)
    ...
    def init_output(self):
        ...
        self.rank_output.weight.data = self.itm_output.weight.data[1:, :]
        self.rank_output.bias.data = self.itm_output.bias.data[1:]
        # self.rank_output.weight.data = self.itm_output.weight.data
        # self.rank_output.bias.data = self.itm_output.bias.data

...
class RetrievalDataset(Dataset):
    ...
    # unbalanced version for retrieval (for the wit dataset)
    def __init__(self, caption_file, img_tsv_info, img_transform, tokenizer, args, is_train=True, coarse_cap_index=None, coarse_img_index=None):
        """
        ...
        self.image_tsv_file = TSVFile(img_tsv_info)
        self.image_id2row = json.load(open(img_tsv_info+'.id2row.json'))
        ...
        if caption_file.endswith('.pth') or caption_file.endswith('.pt'):
            ...
            for line in open(caption_file, 'r'):
                ...
                if info['img_path'] not in self.image_id2row:
                    continue
                self.captions[info['img_path']] = info['sentences']
                self.idx2url[info['img_path']] = info['image_url']
        else:
    ...
    def get_image(self, image_full_id):
        line = self.image_tsv_file[self.image_id2row[image_full_id]]
        assert line[1] == self.idx2url[image_full_id], 'the image info is not aligned!'
        img = Image.open(BytesIO(base64.b64decode(line[2]))).convert('RGB')
...
def train(args, train_dataset, val_dataset, model, tokenizer):
    ...
    best_score = 0
    # for debug here
    # tmp_input = torch.load('/remote-home/zjli/tmp_debug/wit_info.pt')
    # images = tmp_input['images']
    # text = tmp_input['text']
    # # model = model.to(torch.device('cpu'))
    # outputs = model(images, text['input_ids'][:,:50], text['attention_mask'][:,:50])
    ...
    for epoch in range(int(args.num_train_epochs)):
        for step, (_, batch) in enumerate(train_dataloader):
            ...
            text = tokenizer(list(text), padding='longest', max_length=args.max_seq_length, return_tensors="pt", truncation=True).to(args.device)
            if args.time_debug:
            ...
            # if global_step >= 4940:
            #     model_to_save = model.module if hasattr(model, 'module') else model
            #     torch.save({'images':images, 'text':text, 'model':model_to_save.state_dict()}, '/remote-home/zjli/tmp_debug/wit_info.pt')
...
def main():
    ...
    if args.test_language is None or args.test_language=='all':
        ...
        if args.eval_split == 'val':
            ...
            test_img_tsv = image_info_base['val'][args.test_language]
        elif args.eval_split == 'test':
            args.data_file_val = albef_config['test_file'][args.test_language]
            test_img_tsv = image_info_base['test'][args.test_language]
        else:
            raise NotImplementedError
    ...
    if args.do_train:
        if isinstance(args.data_file_train, dict):
            train_dataset = ConcatDataset([RetrievalDataset(caption_file=args.data_file_train[k], img_tsv_info=train_img_tsv[k], img_transform=train_transform, tokenizer=tokenizer, args=args, is_train=True) for k in args.data_file_train])
        else:
            train_dataset = RetrievalDataset(caption_file=args.data_file_train, img_tsv_info=train_img_tsv, img_transform=train_transform, tokenizer=tokenizer, args=args, is_train=True)
        if args.evaluate_during_training:
            if isinstance(args.data_file_val, dict):
                val_dataset = {k: RetrievalDataset(caption_file=v, img_tsv_info=test_img_tsv[k], img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False) for k,v in args.data_file_val.items()}
            else:
                val_dataset = RetrievalDataset(caption_file=args.data_file_val, img_tsv_info=test_img_tsv, img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False)
        else:
            val_dataset = None
    ...
    if args.do_test or args.do_eval:
        if args.eval_all_checkpoints:
            ...
            # args = restore_training_settings(args)
            if isinstance(args.data_file_val, dict):
                test_dataset = {k:RetrievalDataset(caption_file=v, img_tsv_info=test_img_tsv[k], img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False) for k,v in args.data_file_val.items()}
            else:
                test_dataset = RetrievalDataset(caption_file=args.data_file_val, img_tsv_info=test_img_tsv, img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False)
            checkpoint = args.eval_model_dir
            assert op.isdir(checkpoint)
            logger.info("Evaluate the following checkpoint: %s", checkpoint)

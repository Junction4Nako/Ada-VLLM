...
from oscar.modeling.modeling_vlbert import BiImageBertForRE, BiImageBertForSequenceClassification, ImageBertForRE
...
MODEL_CLASSES = {
    'bert': (BertConfig, ImageBertForRE, BertTokenizer),
}
...
class REDataset(Dataset):
    ...
    def __init__(self, args, name, tokenizer):
        ...
        for item in tqdm(self.examples):
            ...
            if self.use_det:
                d_labels = [compute_iou(n_bbox, b) for b in self.det_box[str(img_id)]]
                self.det_labels.append(d_labels)
                pred_boxes = self.od_labels[int(img_id)]['boxes']
                det_boxes = self.det_box[str(img_id)]
        ...
        upper_bound = upper_bound/len(self.examples)
    ...
    def get_od_labels(self, img_key):
        if self.args.img_feat_format == 'tsv':
            ...
            od_labels = ' '.join(self.od_labels[str(img_key)])
        return od_labels
    ...
    #for epoch in train_iterator:
        ...
        for step, (indexes, batch) in enumerate(train_dataloader):
            ...
            if args.use_det:
                ...
                tmp_labels = batch[3].cpu().numpy()
                for i in range(logits.shape[0]):
                    ind = indexes[i]
                    det_label, det_mat = train_dataset.get_det_label(ind)
                    batch_score += re_score_det1(logits[i], tmp_labels[i], det_label, det_mat)
            else:
                tmp_score = re_score(logits, batch[3]).sum()

...
from oscar.modeling.modeling_vlbert_pretrain import BiBertImgForPreTraining2
...
def main():
    ...
    parser.add_argument('--use_mask_phrase', action='store_true')
    parser.add_argument('--only_multi_phrase', action='store_true')
    ...
    for step, (batch, batch_extra) in enumerate(zip(train_dataloader, train_dataloader_extra), start_iter):
        ...
        def forward_backward(image_features, input_ids_a, input_mask_a, segment_ids_a,lm_label_ids_a, input_ids_b, input_mask_b, 
            ...
            if args.deepspeed:
                ...
                outputs = model_engine(input_ids_a=input_ids_a, token_type_ids_a=segment_ids_a, attention_mask_a=input_mask_a,
                                ...
                                token_type_ids_b=segment_ids_b, attention_mask_b=input_mask_b, masked_lm_labels_b=lm_label_ids_b, phrase_layer=args.wra_layer, phrase_mask_label=phrase_mask_label,
            ...
            if phrase_mask_label is None:
                total_loss, vis_mlm_loss, retrieval_loss, masked_lm_loss, next_sentence_loss, qa_rel_loss, wra_loss = outputs
            else:
                total_loss, vis_mlm_loss, retrieval_loss, masked_lm_loss, next_sentence_loss, mask_phrase_loss, qa_rel_loss, wra_loss = outputs
            if torch.isnan(total_loss):
                tmp_info = {'loss':[total_loss, vis_mlm_loss, retrieval_loss, masked_lm_loss, next_sentence_loss, qa_rel_loss, wra_loss]}
                tmp_info['phrase_index'] = phrase_index
                tmp_info['lm_label_ids_a'] = lm_label_ids_a
                tmp_info['image_index'] = image_index
                tmp_info['qa_is_next'] = qa_is_next
                torch.save(tmp_info, '/opt/tiger/tmp_dir/debug/bi_vinvlout.pth')
                logging.info("nan loss found, saving the loss, input, output now")
                raise ValueError
                return None
            ...
            if phrase_mask_label is None:
                mask_phrase_loss = 0
            elif torch.isnan(mask_phrase_loss):
                mask_phrase_loss = 0
            else:
                mask_phrase_loss = loss_weight * mask_phrase_loss.item()
            ...
            return loss.item(), input_ids_a.size(0), wra_loss, vis_mlm_loss, retrieval_loss, mlm_loss, relation_loss, qa_rel_loss, mask_phrase_loss
        ...
        if args.visual_learning:
            ...
            loss1, nb_tr_example1, wra_loss1, vis_mlm_loss1, retrieval_loss1, mlm_loss1, relation_loss1, qa_rel_loss1, mask_phrase_loss1 = forward_backward(
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            metrics_to_log = {
                ...
                'CLIP_loss': retrieval_loss1+retrieval_loss2, 'rel_loss': relation_loss1+relation_loss2, 'qa_rel_loss': qa_rel_loss1+qa_rel_loss2, 'mask_phrase_loss': mask_phrase_loss1+mask_phrase_loss2}
            }

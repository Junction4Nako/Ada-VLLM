...
from oscar.modeling.modeling_bert import BertImgForPreTraining, VLBertImgForPreTraining, BertImgForPreTraining2
from transformers.pytorch_transformers import (WEIGHTS_NAME, BertConfig,
                                  BertTokenizer)

from oscar.ecom_pretrain_dataset.build import make_data_loader, make_data_loader_ds
...
def main():
    ...
    parser.add_argument('--obj_per_frame', type=int, default=20)
    parser.add_argument('--tag_per_frame', type=int, default=10)
    ...
    if not args.refined:
        tag2id = json.load(open(os.path.join(args.data_dir, 'tags2id.json'), 'r'))
    ...
    if args.visual_learning:
        ...
        model_class = BertImgForPreTraining2
    load_num = 0
    ...
    if arguments['iteration'] > 0 and os.path.isfile(os.path.join(last_checkpoint_dir, 'optimizer.pth')) and not args.deepspeed:  # recovery
        logger.info(
    ...
    for step, (batch, batch_extra) in enumerate(zip(train_dataloader, train_dataloader_extra), start_iter):
        ...
        def data_process(mini_batch):
            ...
            input_ids = mini_batch[1].to(c_device, non_blocking=True)
            input_mask = mini_batch[2].to(c_device, non_blocking=True)
            segment_ids = mini_batch[3].to(c_device, non_blocking=True)
            lm_label_ids = mini_batch[4].to(c_device, non_blocking=True)
            is_next = mini_batch[5].to(c_device, non_blocking=True)
            is_img_match = mini_batch[6].to(c_device, non_blocking=True)
            ...
            if images is not None:
                ...
                if args.visual_learning:
                    target_img_feat = mini_batch[7].to(c_device, non_blocking=True)
                    target_img_feat = target_img_feat.to(dtype=args.dtype)
                    visual_labels = mini_batch[8].to(c_device, non_blocking=True)
                    mask_region_id = mini_batch[9].to(c_device, non_blocking=True)
                    return images, input_ids, input_mask, segment_ids, lm_label_ids, is_next, \
                        target_img_feat, visual_labels, mask_region_id
            # my_time_3 = time.time()
        ...
        if args.visual_learning:
            images1, input_ids1, input_mask1, segment_ids1, lm_label_ids1, is_next1, \
                target_img_feat1, visual_labels1, mask_region_id1 = data_process(batch)
        else:
            images1, input_ids1, input_mask1, segment_ids1, lm_label_ids1, is_next1 \
                = data_process(batch)
        ...
        def forward_backward(image_features, input_ids, input_mask, segment_ids,
        ...
        start1 = time.time()
        if args.visual_learning:
            loss1, nb_tr_example1 = forward_backward_v(

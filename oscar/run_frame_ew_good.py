...
class ClassificationDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, data_in, is_train=True, i2v=None, v2s=None, vid2count=None):
        """
        ...
        self.sample_count = 0
        self.data_time = [0]*4
        self.vid2count = vid2count
        # print(self.vid2count is None)
        ...
        self.max_img_seq_len = args.num_frames * args.obj_per_frame
        self.args = args
        ...
        if self.args.add_od_labels:
            if img_key in self.v2s:
                ...
                try:
                    ...
                    ii += '.jpg'
                    image_id = vid+'.jpg'
                    od_tags = self.frame_tag_reader.read_many([image_id])[0]
                except:
                    print('expected split {} but not found {}'.format(self.v2s[img_key], img_key))
                    raise ValueError
    ...
    def get_full_tags(self, img_key):
        if img_key.endswith('.jpg'):
            ...
            ii = img_key
        vid = self.i2v[ii]
        target_keys = [vid+'_'+str(i) for i in range(self.vid2count[vid])]
        # return_values = self.frame_tag_reader.read_many(target_keys)
        res = []
        for k in target_keys:
            v = self.frame_tag_reader.read_many([k])[0]
    ...
    def tensorize_example(self, text_a, img_feat, text_b=None, text_c=None,
        ...
        if text_b:
            ...
            if len(tokens_b) > self.args.max_seq_length - len(tokens) - 1:
                num_left_tokens = max(0, self.args.max_seq_length - len(tokens) - 1) # to avoid -1
                assert(num_extra_tokens >= 0)
    ...
    def __getitem__(self, index):
        ...
        time5 = time.time()
        self.data_time[0] += time2 - time1
        self.data_time[1] += time3 - time2
        self.data_time[2] += time4 - time3
        self.data_time[3] += time5 - time4
        ...
        if self.args.ex_con:
            ...
            text_c = None
        example = self.tensorize_example(caption, frame_feat, text_b=all_tags, text_c=text_c)
        self.sample_count += 1
    ...
    def get_image(self, image_id):
        if image_id in self.v2s:
            ...
            try:
                ...
                ii += '.jpg'
                image_id = vid+'.jpg'
                t_features = self.frame_img_reader.read_many([image_id])[0]
            except:
                print('expected split {} but not found {}'.format(self.v2s[ii], image_id))
                raise ValueError
            pass # need to add judge if cover is combined with frames
        else:  
    ...
    def get_frame_feat(self, image_id):
        ...
        for v in return_values:
            # v = self.frame_img_reader.read_many([k])[0]
            t_features = np.frombuffer(v, dtype=np.float32).reshape(-1, self.args.img_feature_dim)
            if not t_features.flags['WRITEABLE']:
                new_features = np.copy(t_features)
            else:
                new_features = t_features
            if new_features.shape[0] == 1 and np.mean(new_features) == 0:
                continue
        ...
        if len(res) == 0:
            res = [torch.zeros(1, self.args.img_feature_dim)]
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (args.save_steps > 0 and global_step % args.save_steps == 0) or \
                    ...
                    if args.evaluate_during_training: 
                        ...
                        eval_result,_ = evaluate(test_result, by_cate=True)
                        eval_result2,_ = evaluate(test_result2, by_cate=True)
            ...
            for i in range(len(c_precision)):
                if not a_flag:
                    try:
                        ...
                        tmp_info['precision']= 1.0 #c_precision[len(c_precision)//2]
                        tmp_info['recall'] = 0.0 # c_recall[len(c_precision)//2]
                        a_flag = True
                if not b_flag:
...
def main():
    ...
    parser.add_argument('--vid2count', type=str, default=None, help='the vid2frame_num mapping file name')
    ...
    parser.add_argument('--focal_gamma', type=float, default=0, help='gamma for focal loss')
    ...
    global item2cate
    with open(args.item2cate, 'r') as rf:
        item2cate = json.load(rf)
    ...
    #     data_split = json.load(open(args.data_split, 'r'))
    #     assert args.train_split in ['old', 'new', 'all']
    #     if args.train_split == 'old':
    #         train_ids = set([line[0] for line in data_split['train_old']])
    #     elif args.train_split == 'new':
    #         train_ids = set([line[0] for line in data_split['train_new']])
    #     else:
    #         train_ids = set([line[0] for line in data_split['train_old']+data_split['train_new']])
    #     test_2w_ids = set([line[0] for line in data_split['test_2w']])
    #     test_6w_ids = set([line[0] for line in data_split['test_6w']])
    #     test_3w_ids = set([line[0] for line in data_split['test_3w']])
    if args.data_split is not None:
        data_split = json.load(open(args.data_split, 'r'))
        assert args.train_split in ['old', 'new', 'all']
        # sample strategy
        train_ids_old = set()
        train_ids_new = set()
        for line in data_split['train_old']:
            if item2cate[line[0]] == '美食' and args.old_food_sample:
                continue
            train_ids_old.add(line[0])
        for line in data_split['train_new']:
            if item2cate[line[0]] == '时尚':
                if random.random() > args.new_fashion_prop:
                    # sub sample with probability of args.new_fashion_prop
                    continue
            train_ids_new.add(line[0])
        if args.train_split == 'old':
            train_ids = train_ids_old
            # train_ids = set([line[0] for line in data_split['train_old']])
        elif args.train_split == 'new':
            train_ids = train_ids_new
            # train_ids = set([line[0] for line in data_split['train_new']])
        else:
            train_ids = set.union(train_ids_new, train_ids_old)
            # train_ids = set([line[0] for line in data_split['train_old']+data_split['train_new']])
        test_2w_ids = set([line[0] for line in data_split['test_2w']])
        test_6w_ids = set([line[0] for line in data_split['test_6w']])
        test_3w_ids = set([line[0] for line in data_split['test_3w']])
    ...
    with open(args.vid2count, 'r') as rf:
        my_vid2count = json.load(rf)

    assert(args.vid2split is not None)
    v2s = json.load(open(args.vid2split, 'r'))
    ...
    for line in data_csv:
        if line[0] not in my_item2vid:
            continue
        if my_item2vid[line[0]] not in my_vid2count:
            continue
        ...
        if args.ex_con:
            ...
            # print(tmp)
            new_line.append(tmp)
        if line[0] in test_2w_ids:
            val_data_2w.append(new_line)
        elif line[0] in test_6w_ids:
            val_data_6w.append(new_line)
        elif line[0] in test_3w_ids:
            val_data_3w.append(new_line)
        elif line[0] in train_ids:
            if random.random()<args.dev_prop:
                dev_data.append(new_line)
            else:
                train_data.append(new_line)

    train_bili = Counter([line[1] for line in train_data])
    test2w_bili = Counter([line[1] for line in val_data_2w])
    test6w_bili = Counter([line[1] for line in val_data_6w])
    test3w_bili = Counter([line[1] for line in val_data_3w])
    dev_bili = Counter([line[1] for line in dev_data]) if len(dev_data)>0 else None
    print("training ratio:", train_bili.most_common())
    print("test 2w retio:", test2w_bili.most_common())
    print("test 6w retio:", test6w_bili.most_common())
    if dev_bili is not None:
        print("dev ration", dev_bili.most_common())

    if args.do_train:
        train_dataset = ClassificationDataset(tokenizer, args, train_data, is_train=True, i2v=my_item2vid, v2s=v2s, vid2count=my_vid2count)
        if args.evaluate_during_training:
            val_dataset_2w = ClassificationDataset(tokenizer, args, val_data_2w, is_train=False, i2v=my_item2vid, v2s=v2s, vid2count=my_vid2count)
            val_dataset_6w = ClassificationDataset(tokenizer, args, val_data_3w, is_train=False, i2v=my_item2vid, v2s=v2s, vid2count=my_vid2count)
        else:
            val_dataset = None
        global_step, avg_loss = train(args, train_dataset, [val_dataset_2w, val_dataset_6w], model, tokenizer)
    ...
    if args.do_test or args.do_eval:
        ...
        if args.test_split == 'new_train':
            assert args.train_split == 'new'
            test_dataset = ClassificationDataset(tokenizer, args, train_data, is_train=False, i2v=my_item2vid, v2s=v2s, vid2count=my_vid2count)
        else:
            test_dataset_3w = ClassificationDataset(tokenizer, args, val_data_3w, is_train=False, i2v=my_item2vid, v2s=v2s, vid2count=my_vid2count)
            test_dataset_2w = ClassificationDataset(tokenizer, args, val_data_2w, is_train=False, i2v=my_item2vid, v2s=v2s, vid2count=my_vid2count)
            test_dataset_6w = ClassificationDataset(tokenizer, args, val_data_6w, is_train=False, i2v=my_item2vid, v2s=v2s, vid2count=my_vid2count)
        checkpoint = args.eval_model_dir
        assert op.isdir(checkpoint)
        logger.info("Evaluate the following checkpoint: %s", checkpoint)
        model = model_class.from_pretrained(checkpoint, config=config)
        ...
        if args.test_split == 'new_train':
            ...
            test_result = {test_dataset.get_item_id(k):(v, test_dataset.get_label(k), item2cate[test_dataset.get_item_id(k)]) for k,v in test_result.items()}
            torch.save(test_result, args.eval_model_dir + '/predictions-new_train.pth')
            eval_result,by_cate_res = evaluate(test_result, th=args.threshold, by_cate=True)
            result_file = args.eval_model_dir + 'new_train.eval.json'
            with open(result_file, 'w') as f:
                tmp = {'global':eval_result, 'by_cate':by_cate_res}
                json.dump(tmp, f)
            logger.info("Evaluation results saved to {}.".format(result_file))
        else:
            all_eval_splits = args.test_split.split('+')
            if '3w' in all_eval_splits:
                test_result3 = test(args, model, test_dataset_3w)
                test_result3 = {test_dataset_3w.get_item_id(k):(v, test_dataset_3w.get_label(k), item2cate[test_dataset_3w.get_item_id(k)]) for k,v in test_result3.items()}
                torch.save(test_result3, args.eval_model_dir + '/predictions-test_3w.pth')
                eval_result3,by_cate_res3 = evaluate(test_result3, th=args.threshold, by_cate=True)
                result_file3 = args.eval_model_dir + '/test_3w.eval.json'
                with open(result_file3, 'w') as f:
                    tmp = {'global':eval_result3, 'by_cate':by_cate_res3}
                    json.dump(tmp, f)
                logger.info("Evaluation results saved to {}.".format(result_file3))

            if '2w' in all_eval_splits:
                test_result = test(args, model, test_dataset_2w)
                test_result = {test_dataset_2w.get_item_id(k):(v, test_dataset_2w.get_label(k), item2cate[test_dataset_2w.get_item_id(k)]) for k,v in test_result.items()}
                torch.save(test_result, args.eval_model_dir + '/predictions-test_2w.pth')
                eval_result,by_cate_res = evaluate(test_result, th=args.threshold, by_cate=True)
                result_file = args.eval_model_dir + '/test_2w.eval.json'
                with open(result_file, 'w') as f:
                    tmp = {'global':eval_result, 'by_cate':by_cate_res}
                    json.dump(tmp, f)
                logger.info("Evaluation results saved to {}.".format(result_file))

            if '6w' in all_eval_splits:
                test_result2 = test(args, model, test_dataset_6w)
                test_result2 = {test_dataset_6w.get_item_id(k):(v, test_dataset_6w.get_label(k), item2cate[test_dataset_6w.get_item_id(k)]) for k,v in test_result2.items()}
                torch.save(test_result2, args.eval_model_dir + '/predictions-test_6w.pth')
                eval_result2,by_cate_res2 = evaluate(test_result2, th=args.threshold, by_cate=True)
                result_file2 = args.eval_model_dir + '/test_6w.eval.json'
                with open(result_file2, 'w') as f:
                    tmp = {'global':eval_result2, 'by_cate':by_cate_res2}
                    json.dump(tmp, f)
                logger.info("Evaluation results saved to {}.".format(result_file2))

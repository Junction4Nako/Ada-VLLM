        ...
        if self.args.add_od_labels:
            if img_key in self.v2s:
                ...
                try:
                    ...
                    ii += '.jpg'
                    image_id = vid+'.jpg'
                    od_tags = self.frame_tag_reader[self.v2s[ii]-1].read_many([image_id])[0]
                except:
    ...
    def get_image(self, image_id):
        if image_id in self.v2s:
            ...
            try:
                ...
                ii += '.jpg'
                image_id = vid+'.jpg'
                t_features = self.frame_img_reader[self.v2s[ii]-1].read_many([image_id])[0]
            except:
...
def worker_init_fn(_):
    ...
    dataset.frame_img_reader = []
    dataset.frame_tag_reader = []
    for i in range(dataset.num_splits):
        if i < 6:
            dataset.frame_img_reader.append(KVReader(dataset.frame_path+'split'+str(i+1)+'_obj_feats', dataset.num_readers))
            dataset.frame_tag_reader.append(KVReader(dataset.frame_tag_path+'split'+str(i+1)+'_od_tags', dataset.num_readers))
        else:
            dataset.frame_img_reader.append(KVReader(dataset.frame_path+'split'+str(i+1)+'obj_feats', dataset.num_readers))
            dataset.frame_tag_reader.append(KVReader(dataset.frame_tag_path+'split'+str(i+1)+'od_tags', dataset.num_readers))
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (args.save_steps > 0 and global_step % args.save_steps == 0) or \
                    ...
                    if args.evaluate_during_training: 
                        ...
                        eval_result = evaluate(test_result, by_cate=True)
                        eval_result2 = evaluate(test_result2, by_cate=True)
...
def main():
    ...
    if args.do_test or args.do_eval:
        ...
        test_dataset = ClassificationDataset(tokenizer, args, val_data, is_train=False, i2v=my_item2vid, v2s=v2s)
        checkpoint = args.eval_model_dir
        assert op.isdir(checkpoint)
        logger.info("Evaluate the following checkpoint: %s", checkpoint)
        model = model_class.from_pretrained(checkpoint, config=config)
        ...
        eval_result = evaluate(test_result, th=args.threshold, by_cate=True)

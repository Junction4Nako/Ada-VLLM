...
class RetrievalDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, split='train', is_train=True, reranking_mode='i2t'):
        ...
        if is_train:
            ...
            if (args.eval_caption_index_file or args.train_sub_index) and reranking_mode=='i2t':
    ...
    def get_image_caption_index(self, index):
        ...
        if not self.is_train and self.args.less_cap_train and self.split=='train':
    ...
    def tensorize_example(self, text_a, img_feat, text_b=None, text_c=None,
        ...
        if text_c is not None:
            ...
            concept_len = len(text_c)
            concept_span = [seq_a_len, seq_a_len+concept_len]
        ...
        attention_mask = torch.tensor(attention_mask, dtype=torch.long)
        segment_ids = torch.tensor(segment_ids, dtype=torch.long)
        concept_span = torch.tensor(concept_span, dtype=torch.long)
        return (input_ids, attention_mask, segment_ids, img_feat, concept_span)
    ...
    def __len__(self):
        ...
        if not self.is_train and self.args.less_cap_train:
            return len(self.img_keys) * self.num_captions_per_img
...
def main():
    ...
    if args.do_test or args.do_eval:
        ...
        train_dataset = RetrievalDataset(tokenizer, args, 'train', is_train=False)
        checkpoint = args.eval_model_dir
        assert op.isdir(checkpoint)
        logger.info("Evaluate the following checkpoint: %s", checkpoint)
        model = model_class.from_pretrained(args.model_name_or_path, config=config)
        ...
        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, 
                batch_size=args.per_gpu_train_batch_size, num_workers=args.num_workers)
        print('length of dataloader', len(train_dataloader))
        concept_span = [args.max_seq_length, args.max_seq_length+args.num_extra_concept]
        # print(concept_span)
        conid2emb = {}
        pad_id = tokenizer.convert_tokens_to_ids([tokenizer.pad_token])[0]
        with torch.no_grad():
            for in_index, batch in tqdm(train_dataloader):
                # print(in_index)
                model_engine.eval()
                batch = tuple(t.to(model_engine.device) for t in batch)
                inputs = {
                ...
                inputs = prepare_inputs(inputs, args)
                outputs = model(**inputs)
                concep_emb = outputs[-1]
                # print(concep_emb.shape)
                result = distributed_concat(concep_emb.contiguous())
                # print(result.shape)
                concep_index = []
                for i in range(batch[0].shape[0]):
                    concep_index.append(batch[0][i, batch[5][i,0]:batch[5][i,1]])
                concep_index = torch.cat([c.contiguous() for c in concep_index], dim=0)
                # print(concep_index.shape)
                concep_index = distributed_concat(concep_index.contiguous())
                # print(concep_index.shape)
                # print(result.shape)
                result = result.detach().cpu().numpy()
                concep_index = concep_index.detach().cpu().numpy()
                for i in range(concep_index.shape[0]):
                    for j in range(concep_index.shape[1]):
                        c_id = concep_index[i,j]
                        if c_id != pad_id:
                            if c_id not in conid2emb:
                                conid2emb[c_id] = [1, result[i, j, :]]
                            else:
                                c_nums = conid2emb[c_id][0]
                                conid2emb[c_id][1] = (c_nums/(c_nums+1))*conid2emb[c_id][1] + result[i, j, :]/(c_nums+1)
                                conid2emb[c_id][0] += 1
        torch.save(conid2emb, op.join(checkpoint, 'train_concept_emb.pt'))

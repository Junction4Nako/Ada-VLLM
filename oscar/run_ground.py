...
from oscar.modeling.modeling_vlbert_pretrain import BiImageBertForGrounding, BiImageBertForSequenceClassification
...
MODEL_CLASSES = {
    'bert': (BertConfig, BiImageBertForGrounding, BertTokenizer),
}
...
class GroundingDataset(Dataset):
    ...
    def __init__(self, args, name, tokenizer, instances=None, entity2box=None):
        ...
        self.full_labels = {int(item['id']):[[] for i in range(5)] for item in self.data}
        for i,ex in enumerate(self.examples):
            self.full_labels[ex['image_id']][ex['cap_id']].append(i)
    ...
    def tensorize_example_group(self, img_key, caption_info, tmp_label, phrases, cls_token_at_end=False, pad_on_left=False,
                    cls_token='[CLS]', sep_token='[SEP]', pad_token=0,
                    sequence_a_segment_id=0, sequence_b_segment_id=1,
                    cls_token_segment_id=1, pad_token_segment_id=0,
                    mask_padding_with_zero=True):

        tokens_a = self.tokenizer.tokenize(caption_info['sentence'])
        num_extra_tokens = 2
        num_phrases = self.args.max_phrases
        full_indexes = []
        for example in caption_info['phrases']:
            start_idx = example['start']
            end_idx = example['end']
            if end_idx > self.args.max_seq_length-num_extra_tokens:
                start_idx = 0
                end_idx = 0
            indexs = torch.tensor([0 for i in range(self.args.max_seq_length + self.args.max_phrases)], dtype=torch.float32)
            indexs[(start_idx+1):(end_idx+1)] = 1
            full_indexes.append(indexs)
        assert len(full_indexes) == len(tmp_label)
        if len(full_indexes) == 0:
            full_indexes = torch.zeros(1, self.args.max_seq_length + self.args.max_phrases).to(torch.float32)
            tmp_label = [[-1]]
        else:
            full_indexes = torch.stack(full_indexes)
        # padding to max phrases
        index_to_pad = torch.zeros(self.max_phrase-full_indexes.shape[0], full_indexes.shape[1]).to(torch.float32)
        full_indexes = torch.cat([full_indexes, index_to_pad], dim=0)
        tmp_label = torch.tensor(tmp_label, dtype=torch.float32)
        label_to_pad = torch.ones(self.max_phrase-tmp_label.shape[0], tmp_label.shape[1]).to(torch.float32)*(-1)
        tmp_label = torch.cat([tmp_label, label_to_pad], dim=0)
        ...
        if img_feat.shape[0] > self.args.max_img_seq_length:
            ...
            img_feat = torch.cat((img_feat, padding_matrix), 0)
            input_mask_b = input_mask_b + ([0] * padding_matrix.shape[0])
        if  tmp_label.shape[1] > self.args.max_img_seq_length:
            tmp_label = tmp_label[:, :self.args.max_img_seq_length]
        else:
            tmp_label_pad = torch.ones(tmp_label.shape[0], self.args.max_img_seq_length - tmp_label.shape[1]).to(torch.float32)*(-1)
            tmp_label = torch.cat([tmp_label, tmp_label_pad], dim=1)
                # segment_ids = segment_ids + [pad_token_segment_id] * padding_matrix.shape[0]
        ...
        # print('label shape:', tmp_label.shape)
        # print('full indexes:', full_indexes.shape)
        return (torch.tensor(input_ids_a, dtype=torch.long),
                torch.tensor(input_mask_a, dtype=torch.long),
                torch.tensor(segment_ids_a, dtype=torch.long),
                torch.tensor(input_ids_b, dtype=torch.long),
    ...
    def __getitem__(self, index):
        if self.name == 'train':
            img_index = index // 5
            cap_index = index % 5
            c_data = self.data[img_index]
            c_caption = c_data['captions'][cap_index]
            img_key = int(c_data['id'])
            tmp_label = [self.labels[i] for i in self.full_labels[img_key][cap_index]]
            # print(tmp_label)
            phrases = self.get_caption_phrase(img_key, cap_index)
            example = self.tensorize_example_group(img_key, c_caption, tmp_label, phrases,
...
def gather_logits_scores(logits, scores, num_per_sample):
    full_logits = []
    full_scores = []
    for idx in range(logits.shape[0]):
        valid_index = torch.arange(0, num_per_sample[idx], device=logits.device)
        full_logits.append(torch.index_select(logits[idx], dim=0, index=valid_index))
        full_scores.append(torch.index_select(scores[idx], dim=0, index=valid_index))
    return torch.cat(full_logits, dim=0), torch.cat(full_scores, dim=0)
...
def train(args, train_dataset, eval_dataset, model, tokenizer, test_ds=None):
    ...
    #for epoch in train_iterator:
        ...
        for step, (indexes, batch) in enumerate(train_dataloader):
            ...
            num_phrases = torch.sum(batch[8], dim=-1)
            phrase_mask = num_phrases > 0
            num_phrase_per_sample = torch.sum(phrase_mask, dim=1)
            full_logits, full_scores = gather_logits_scores(logits, batch[6], num_phrase_per_sample)
            tmp_score = re_score(full_logits, full_scores)
            

            batch_score =  (tmp_score>=0.5).int().sum().item() / full_logits.shape[0]

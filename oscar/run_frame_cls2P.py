...
class ClassificationDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, data_in, is_train=True, i2v=None):
        ...
        self.img_feat_path = [args.img_feat_path, args.L3_hdfs+'obj_feats']
        self.num_readers = args.num_readers
        
        if args.add_od_labels:
            self.od_tag_path = [args.od_tag_path, args.L3_hdfs+'od_tags']
    ...
    def get_od_labels(self, img_key, split=0):
        ...
        if self.args.add_od_labels:
            od_tags = self.od_reader[split].read_many([img_key])[0]
    ...
    def get_full_tags(self, img_key, split=0):
        if img_key.endswith('.jpg'):
            ii = img_key[:-4]
        else:
            ii = img_key
        ...
        return_values = self.od_reader[split].read_many(target_keys)
        res = []
    ...
    def __getitem__(self, index):
        ...
        split = c_data[-1]
        img_feat = self.get_image(img_key, split)
        od_tags = self.get_od_labels(img_key, split)
        all_tags = self.get_full_tags(img_key, split)
        od_tags = od_tags + ' ' + all_tags
        frame_feat = self.get_frame_feat(img_key, split)
        img_feat = torch.cat([img_feat, frame_feat], dim=0)
        caption = c_data[2]
        label = c_data[1]
        if self.args.less_prob < 1 and self.is_train:
            if split == 0:
                label = self.args.less_prob*label
            else:
                label = float(label)
        if self.args.ex_con:
            text_c = c_data[-2]
    ...
    def get_image(self, image_id, split=0):
        ...
        vid = self.i2v[ii]
        image_id = vid+'.jpg'
        t_features = self.img_reader[split].read_many([image_id])[0]
    ...
    def get_frame_feat(self, image_id, split):
        if image_id.endswith('.jpg'):
            ii = image_id[:-4]
        else:
            ii = image_id
        vid = self.i2v[ii]
        target_keys = [vid+'_'+str(i)+'.jpg' for i in range(self.args.num_frames)]
        return_values = self.img_reader[split].read_many(target_keys)
        res = []
...
def worker_init_fn(_):
    ...
    dataset.img_reader = []
    dataset.od_reader = []
    for i_d in dataset.img_feat_path:
        dataset.img_reader.append(KVReader(i_d, dataset.num_readers))
    for o_d in dataset.od_tag_path:
        dataset.od_reader.append(KVReader(o_d, dataset.num_readers))
...
def get_loss_weight(args):
    raw_weights = torch.FloatTensor([0.3, 0.7])
...
def main():
    ...
    parser.add_argument('--L3_hdfs', type=str, default=None)
    parser.add_argument('--L3_i2v', type=str, default=None)
    parser.add_argument('--L3_table', type=str, default=None)
    parser.add_argument('--less_prob', type=float, default=1)
    ...
    for line in data_csv:
        ...
        if args.ex_con:
            ...
            # print(tmp)
            new_line.append(tmp)
        new_line.append(0) # 0 indicate not L3
    ...
    L3_table = csv.reader(open(args.L3_table, 'r'))
    L3_i2v = json.load(open(args.L3_i2v, 'r'))
    next(L3_table)
    L3_item2tag = {}
    for line in L3_table:
        if line[0] not in L3_i2v:
            continue
        tags = [t for t in line[-4:] if len(t)>0 and t!='NULL']
        tags = [json.loads(t)['tag_name'] for t in tags]
        if len(tags) <= 2:
            continue
        else:
            if tags[1] not in ['彩妆', '护肤', '美妆']:
                continue
        L3_item2tag[line[0]] = ';'.join(tags)
        tags = [tag2id[i] for i in tags if i in tag2id]
        new_line = [line[0], 1, line[1], tags, 1] # 1 inidate L3_plus
        train_data.append(new_line)
    my_item2vid.update(L3_i2v)
    item2tag.update(L3_item2tag)

...
def main():
    ...
    parser.add_argument('--iter_pf', action='store_true', help='whether to iterate all possile')
    ...
    if args.do_test or args.do_eval:
        ...
        test_dataset = RetrievalDataset(tokenizer, args, args.test_split, is_train=False, img_feat=train_img_feat)
        if args.eval_image_index_file:
            test_dataset2 = RetrievalDataset(tokenizer, args, args.test_split, is_train=False, reranking_mode='t2i', img_feat=train_img_feat)
        else:
            test_dataset2 = None
        checkpoint = args.eval_model_dir
        assert op.isdir(checkpoint)
        ...
        if args.do_eval:
            if args.iter_pf:
                ...
                for pf in range(10):
                    ...
                    if test_dataset2:
                        t2i_result_tmp = evaluate(test_dataset2, t2i_result, pr_factor=0.1*pf)
                        eval_result.update(t2i_result_tmp)

...
import random, json
import numpy as np
from oscar.modeling.modeling_bert import ImageBertForSequenceClassification_ML
import torch
...
class ClassificationDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # tmp_label = torch.zeros(len(self.args.label_mapping))
        # tmp_label[label] = 1
        if self.args.ex_con:
            text_c = c_data[-1]
        else:
        ...
        return index, tuple(list(example) + [tmp_label])
...
def train(args, train_dataset, val_dataset, model, tokenizer):
    ...
    for epoch in range(int(args.num_train_epochs)):
        for step, (_, batch) in enumerate(train_dataloader):
            ...
            # print({k:v.dtype for k,v in inputs.items()})
            if args.loss_weight:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (args.save_steps > 0 and global_step % args.save_steps == 0) or \
                    ...
                    if args.evaluate_during_training: 
                        ...
                        test_result = {val_dataset.get_item_id(k):v for k,v in test_result.items()}
                        torch.save(test_result, args.output_dir + '/predictions-{}-{}.pth'.format(epoch, global_step))
                        eval_result = evaluate(test_result, label_mapping=args.label_mapping)
    ...
    for indexs, batch in tqdm(eval_dataloader):
        ...
        with torch.no_grad():
            ...
            for i, idx in enumerate(indexs):
                results[idx.item()] = [result[i,:], labels[i,:]]
            # results.update({idx.item(): res.item() for idx, res in zip(indexs, result)})
    return results


def evaluate(test_results, th = None, label_mapping=None):
    total = 0
    ...
    for k,v in test_results.items():
        total += 1
        full_cate_label.append(v[1])
        full_cate_preds.append(v[0])
        for i in range(len(v[0])):
            ...
            pred = int(logit > th)
            res[(label, pred)] += 1
    # full_cate_label = np.stack(full_cate_label)
    # full_cate_preds = np.stack(full_cate_preds)

    acc = (res[(0, 0)]+res[(1, 1)])/total
    prec = float(res[(1, 1)]/(res[(0, 1)]+res[(1, 1)]+0.001))
    recall = float(res[(1, 1)]/(res[(1, 0)]+res[(1, 1)]+0.001))
    ...
    all_r_labels = np.stack(full_cate_label)
    all_r_preds = np.stack(full_cate_preds)
    cate2pr = []
    for i in range(all_r_labels.shape[1]):
        c_preds = all_r_preds[:, i]
        c_labels = all_r_labels[:, i]
        c_precision, c_recall, c_threshs = precision_recall_curve(c_labels, c_preds)
        fpr, tpr, thresholds = roc_curve(c_labels, c_preds, pos_label=1)
        tmp_info = {}
        ...
        cate2pr.append(tmp_info)
    if label_mapping is not None:
        cate2pr = {label_mapping[i]:t for i,t in enumerate(cate2pr)}
    logger.info(cate2pr)
...
def get_loss_weight(args):
    if hasattr(args, 'pos_weight'):
        raw_weights = torch.FloatTensor(args.pos_weight)
        return torch.cat([raw_weights]*args.n_gpu, dim=0)
    if args.pos_neg == 2:
...
def main():
    ...
    parser.add_argument('--label_mapping', type=str, default=None, help='multi-label mapping file')
    parser.add_argument('--visual_random', action='store_true')
    args = parser.parse_args()

    assert args.label_mapping is not None
    if args.label_mapping.endswith('.json'):
        args.label_mapping = json.load(open(args.label_mapping, 'r'))
    else:
        # a file containing the category name per line
        with open(args.label_mapping, 'r') as rf:
            args.label_mapping = [line.strip() for line in rf.readlines()]
    ...
    model_class = ImageBertForSequenceClassification_ML
    ...
    next(data_csv)
    label2bili = [[0,0] for _ in range(len(args.label_mapping))]
    for line in data_csv:
        ...
        # labels = [args.label_mapping[l]['label_index'] for l in labels if l in args.label_mapping]
        new_line.append(labels)
        if args.ex_con:
        ...
        if line[0] in test_ids:
            ...
            if random.random()<args.dev_prop:
                ...
                for i in range(len(args.label_mapping)):
                    if labels[i] == 1:
                        label2bili[i][1] += 1
                    else:
                        label2bili[i][0] += 1
    ...
    args.pos_weight = []
    cate2bili = {}
    # all_cates = list(args.label_mapping.values())
    for i in range(len(args.label_mapping)):
        if label2bili[i][1] == 0 or label2bili[i][0] == 0:
            args.pos_weight.append(1)
        else:
            args.pos_weight.append(min(10, label2bili[i][0]/label2bili[i][1]))
        cate2bili[args.label_mapping[i]] = label2bili[i]
    ...
    if args.do_eval:
        # do evaluation
        eval_result = evaluate(test_result, th=args.threshold, label_mapping=args.label_mapping)

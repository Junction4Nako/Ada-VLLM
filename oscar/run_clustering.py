...
from oscar.modeling.modeling_bert import ImageBertEmbedding
...
def test(args, model, eval_dataset):
    ...
    for indexs, batch in tqdm(eval_dataloader):
        ...
        with torch.no_grad():
            ...
            embeddings = [e.detach().cpu() for e in embeddings]
            # result = [_.to(torch.device("cpu")) for _ in result]
            results.update({idx.item(): res for idx, res in zip(indexs, embeddings)})
    return results


def evaluate(test_results, args, th = None):
    full_embs = []
    clus_ids = []
    for k,v in test_results.items():
        full_embs.append(v[0])
        clus_ids.append(v[1])
    full_embs = torch.stack(full_embs)
    clus_ids = torch.tensor(clus_ids)
    n_ins = full_embs.shape[0]
    print(full_embs.shape)
    labels = clus_ids.repeat([n_ins, 1]).eq(clus_ids.unsqueeze(1))
    # preds = torch.sigmoid(full_embs @ full_embs.t())
    if args.sim_type == 'cosine':
        full_embs = F.normalize(full_embs, p=2, dim=-1)
        preds = full_embs @ full_embs.t()
    elif args.sim_type == 'inner_product':
        preds = torch.sigmoid(full_embs @ full_embs.t())
    elif args.sim_type == 'l2':
        preds = torch.zeros_like(labels)
        for i in tqdm(range(n_ins)):
            for j in range(n_ins):
                preds[i,j] = torch.sigmoid(-1*torch.norm(full_embs[i,]-full_embs[j,]))
    else:
        raise ValueError
    select_mask = torch.ones_like(labels)
    select_mask[torch.arange(n_ins), torch.arange(n_ins)] = 0
    select_mask = select_mask.bool()
    labels = torch.masked_select(labels, select_mask).cpu().numpy()
    preds = torch.masked_select(preds, select_mask).cpu().numpy()
    total = 0
...
def main():
    ...
    model_class = ImageBertEmbedding
    ...
    print('training:',tag1.most_common())
    # print('test:', c2.most_common())
    print('test:', tag2.most_common())
    if args.do_train:
    ...
    if args.do_eval:
        # do evaluation
        eval_result = evaluate(test_result, args, th=args.threshold)

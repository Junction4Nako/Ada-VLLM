...
class RetrievalDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, split='train', is_train=True, coarse_cap_index=None, coarse_img_index=None):
        ...
        self.sg2id = {k:int(v) for k,v in self.id2sg.items()}
    ...
    def tensorize_example(self, text_a, img_feat, text_b=None, visual_theme=None, phrase_nodes=None,
        ...
        input_ids = self.tokenizer.convert_tokens_to_ids(tokens) + phrase_nodes + [self.tokenizer.vocab[self.tokenizer.sep_token]]
        segment_ids = [cls_token_segment_id] + [sequence_a_segment_id] * (len(tokens_a) + len(phrase_nodes) + 1)
        seq_a_len = len(input_ids)
        if text_b:
            tokens_b = self.tokenizer.tokenize(text_b)
            if len(tokens_b) > self.max_seq_len - len(tokens) - 2:
                num_left_tokens = max(0, self.max_seq_len - len(tokens) - 2) # to avoid -1
                assert(num_left_tokens >= 0)
                tokens_b = tokens_b[: (self.max_seq_len - len(tokens) - 2)]
            tokens += tokens_b + [self.tokenizer.sep_token]
            input_ids += self.tokenizer.convert_tokens_to_ids(tokens_b) + visual_theme + [self.tokenizer.vocab[self.tokenizer.sep_token]]
            segment_ids += [sequence_b_segment_id] * (len(tokens_b) + len(visual_theme) + 1)

        seq_len = len(input_ids)
        tmp_max_seq_len = self.max_seq_len + self.args.max_visual_themes + self.args.max_phrases
        seq_padding_len = tmp_max_seq_len - seq_len
        if seq_padding_len < 0:
            print(len(tokens_a), len(tokens_b), len(visual_theme), len(phrase_nodes))
            raise ValueError
        input_ids += [self.tokenizer.vocab[self.tokenizer.pad_token]] * seq_padding_len
...
def main():
    ...
    parser.add_argument('--max_visual_themes', type=int, default=5)
    parser.add_argument('--max_phrases', type=int, default=5)
    parser.add_argument('--img2theme', type=str, default=None)
    parser.add_argument('--sent_sg_json', type=str, default=None)
    parser.add_argument('--node2id', type=str, default=None)

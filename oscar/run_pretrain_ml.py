...
from oscar.modeling.modeling_vlbert import BiBertImgForPreTraining, BiBertImgForPreTrainingQA
...
MODEL_CLASSES = {
    'bert': (BertConfig, BiBertImgForPreTrainingQA, BertTokenizer),
}
...
def main():
    ...
    parser.add_argument('--qa_loss', action='store_true')
    parser.add_argument('--qa_mapping', type=str, default=None)
    parser.add_argument('--phrase_mode', type=str, default='sample')
    parser.add_argument('--hn_mod', type=str, default='hard')
    parser.add_argument('--only_qa', action='store_true')
    parser.add_argument('--phrase_layer', type=int, default=None)
    parser.add_argument('--no_wpg', action='store_true')
    parser.add_argument('--no_phrase', action='store_true')
    ...
    if args.qa_loss:
        assert args.qa_mapping is not None
        tmp_qa_mapping = json.load(open(args.qa_mapping, 'r'))
        config.qa_answer_size = len(tmp_qa_mapping)
    else:
        config.qa_answer_size = 1
    ...
    if args.visual_learning:
        ...
        model_class = BiBertImgForPreTrainingQA
    load_num = 0
    ...
    for step, (batch, batch_extra) in enumerate(zip(train_dataloader, train_dataloader_extra), start_iter):
        ...
        def data_process(mini_batch):
            ...
            if args.no_wpg:
                phrase_index, image_index = None, None
            if args.qa_loss:
                qa_ans = mini_batch[13].to(c_device, non_blocking=True)
            else:
                qa_ans = None
            ...
            return images, input_ids_a, input_mask_a, segment_ids_a, lm_label_ids_a, input_ids_b, input_mask_b, segment_ids_b, lm_label_ids_b, is_next, is_img_match, phrase_index, image_index, qa_ans

        images1, input_ids_a1, input_mask_a1, segment_ids_a1, lm_label_ids_a1, input_ids_b1, input_mask_b1, segment_ids_b1, lm_label_ids_b1, is_next1, is_img_match, phrase_index, image_index, qa_ans  = data_process(batch)

        data_time = time.time() - end

        def forward_backward(image_features, input_ids_a, input_mask_a, segment_ids_a,lm_label_ids_a, qa_ans,
            ...
            if args.deepspeed:
                ...
                outputs = model_engine(input_ids_a=input_ids_a, token_type_ids_a=segment_ids_a, attention_mask_a=input_mask_a,
                                ...
                                phrase_index=phrase_index, img_index=image_index, max_tag_length=args.max_tag_length, qa_ans=qa_ans,
                                phrase_layer=args.phrase_layer, hn_mod=args.hn_mod, phrase_mod=args.phrase_mode)
            else:
            ...
            if not args.qa_loss:
                if args.no_wpg:
                    total_loss, vis_mlm_loss, retrieval_loss, masked_lm_loss, next_sentence_loss = outputs
                    wra_loss = 0
                else:
                ...
                if args.no_wpg:
                    total_loss, vis_mlm_loss, retrieval_loss, masked_lm_loss, next_sentence_loss, qa_loss = outputs
                    wra_loss = 0
                else:
                    total_loss, vis_mlm_loss, retrieval_loss, masked_lm_loss, next_sentence_loss, qa_loss, wra_loss = outputs
                    wra_loss = loss_weight * wra_loss.item()
                qa_loss = qa_loss.item()*loss_weight
            ...
            return loss.item(), input_ids_a.size(0), wra_loss, vis_mlm_loss, retrieval_loss, mlm_loss, relation_loss, qa_loss
        ...
        if args.visual_learning:
            ...
            loss1, nb_tr_example1, wra_loss1, vis_mlm_loss1, retrieval_loss1, mlm_loss1, relation_loss1, qa_loss1 = forward_backward(
                image_features=images1, input_ids_a=input_ids_a1, input_mask_a=input_mask_a1, qa_ans=qa_ans,
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            metrics_to_log = {
                ...
                'CLIP_loss': retrieval_loss1+retrieval_loss2, 'rel_loss': relation_loss1+relation_loss2, 'qa_loss':qa_loss1+qa_loss2}
            }

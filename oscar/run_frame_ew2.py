...
class ClassificationDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, data_in, is_train=True, i2v=None, v2s=None):
        """
...
def main():
    ...
    if args.do_train:
        train_dataset = ClassificationDataset(tokenizer, args, train_data, is_train=True, i2v=my_item2vid, v2s=v2s)
        if args.evaluate_during_training:
            val_dataset_2w = ClassificationDataset(tokenizer, args, val_data_2w, is_train=False, i2v=my_item2vid, v2s=v2s)
            val_dataset_6w = ClassificationDataset(tokenizer, args, val_data_3w, is_train=False, i2v=my_item2vid, v2s=v2s)
        else:
            val_dataset = None
    ...
    if args.do_test or args.do_eval:
        ...
        if args.test_split == 'new_train':
            ...
            test_dataset = ClassificationDataset(tokenizer, args, train_data, is_train=False, i2v=my_item2vid, v2s=v2s)
        else:
            test_dataset_3w = ClassificationDataset(tokenizer, args, val_data_3w, is_train=False, i2v=my_item2vid, v2s=v2s)
            test_dataset_2w = ClassificationDataset(tokenizer, args, val_data_2w, is_train=False, i2v=my_item2vid, v2s=v2s)
            test_dataset_6w = ClassificationDataset(tokenizer, args, val_data_6w, is_train=False, i2v=my_item2vid, v2s=v2s)
        checkpoint = args.eval_model_dir
        assert op.isdir(checkpoint)
        logger.info("Evaluate the following checkpoint: %s", checkpoint)
        model = model_class.from_pretrained(checkpoint, config=config)

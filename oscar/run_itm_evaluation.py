...
# try to manage it to adapt to deepspeed framework
...
class RetrievalDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, split='train', is_train=True):
        ...
        if args.clip_neg_sampling and is_train:
            neg_scpres_file = op.join(args.data_dir, '{}_clip_scores.pt'.format(split))
        ...
        if self.is_train:
            ...
            if self.args.clip_neg_sampling and random.random() <= 0.4:
                if random.random() <= 0.5:
                    caption_neg = self.get_neg_txt(img_idx)
...
def main():
    ...
    if args.do_test or args.do_eval:
        ...
        logger.info("Evaluate the following checkpoint: %s", checkpoint)
        model = model_class.from_pretrained(args.model_name_or_path, config=config)
        model_engine = deepspeed.init_inference(model=model, mp_size=2, dtype=torch.half, replace_method=None)
        model = model_engine.module

...
class ClassificationDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, data_in, is_train=True, i2v=None):
        ...
        self.total_img_path = args.total_img_path
        self.total_tag_path = args.total_tag_path
    ...
    def get_od_labels(self, img_key, method=1):
        if method == 1:
            c_reader = self.od_reader
        else:
            c_reader = self.total_tag_reader
            if img_key.endswith('.jpg'):
                img_key = img_key[:-4]
            img_key = self.i2v[img_key]+'.jpg'
        if self.args.add_od_labels:
            od_tags = c_reader.read_many([img_key])[0]
    ...
    def get_full_tags(self, img_key, method=1):
        if method == 1:
            c_reader = self.frame_tag_reader
        else:
            c_reader = self.total_tag_reader
        if img_key.endswith('.jpg'):
            ii = img_key[:-4]
        ...
        return_values = c_reader.read_many(target_keys)
        res = []
    ...
    def __getitem__(self, index):
        ...
        img_feat = self.get_image(img_key, method=method)
        od_tags = self.get_od_labels(img_key, method=method)
        all_tags = self.get_full_tags(img_key, method=method)
        od_tags = od_tags + ' ' + all_tags
        frame_feat = self.get_frame_feat(img_key, method=method)
    ...
    def get_image(self, image_id, method=1):
        if method == 1:
            c_reader = self.img_reader
        else:
            c_reader = self.total_img_reader
            if image_id.endswith('.jpg'):
                image_id = image_id[:-4]
            image_id = self.i2v[image_id]+'.jpg'
        t_features = c_reader.read_many([image_id])[0]
    ...
    def get_frame_feat(self, image_id, method=1):
        if method == 1:
            c_reader = self.frame_img_reader
        else:
            c_reader = self.total_img_reader
        if image_id.endswith('.jpg'):
            ii = image_id[:-4]
...
def worker_init_fn(_):
    ...
    dataset.frame_img_reader = KVReader(dataset.frame_path, dataset.num_readers)
    dataset.frame_tag_reader = KVReader(dataset.frame_tag_path, dataset.num_readers)
    dataset.total_img_reader = KVReader(dataset.total_img_path, dataset.num_readers)
    dataset.total_tag_reader = KVReader(dataset.total_tag_path, dataset.num_readers)
...
def evaluate(test_results, th = None):
    ...
    prec = res[(1, 1)]/(res[(0, 1)]+res[(1, 1)]+0.0001)
    recall = res[(1, 1)]/(res[(1, 0)]+res[(1, 1)]+0.0001)
...
def restore_training_settings(args):
    ...
    override_params = ['do_lower_case', 'img_feature_type', 'max_seq_length', 
            ...
            'use_img_layernorm', 'img_layer_norm_eps', 'num_frames', 'obj_per_frame',
            'tag_per_frame', 'refined']
...
def get_loss_weight(args):
    raw_weights = torch.FloatTensor([1.2, 0.4])
...
def main():
    ...
    parser.add_argument('--item2tag2', type=str, default=None, help='old item 2 tags')
    ...
    parser.add_argument('--total_img_path', type=str, default=None, help='joint formula of img features')
    parser.add_argument('--total_tag_path', type=str, default=None, help='joint formula of img tags')
    parser.add_argument('--eval_split', type=str, default='test', help='which split to evaluate on?')
    parser.add_argument('--item2vid2', type=str, default=None, help='old item 2 vid mapping')
    ...
    if args.ex_con:
        ...
        with open(args.item2tag2, 'r') as rf:
            item2tag2 = json.load(rf)
    ...
    if args.item2vid2 is not None:
        my_item2vid2 = json.load(open(args.item2vid2, 'r'))
    else:
        my_item2vid2 = {}
    ...
    for line in data_csv:
        ...
        if line[0] not in test_ids:
            if good_ids is not None and line[0] not in hb_data and line[0] not in good_ids:
                c1 += 1
                continue
        if line[0] not in my_item2vid and line[0] not in my_item2vid2:
            c2 += 1
            continue
        new_line = [info for info in line]
        new_line[1] = int(new_line[1])
        if line[0] in my_item2vid:
            new_line.append(0) # data available in seperate formula (different path for cover images and frames)
        else:
            new_line.append(1) # data availabel in joint formula
        if args.ex_con:
            if line[0] in item2tag:
                tmp = [tag2id[i] for i in item2tag[line[0]].split(';') if i in tag2id]
            else:
                tmp = [tag2id[i] for i in item2tag2[line[0]].split(';') if i in tag2id]
            # print(tmp)
            new_line.append(tmp)
        if line[0] in test_ids:
            ...
            if random.random()<args.dev_prop:
                ...
                if True:
                    ...
                    if new_line[0] in hb_data or random.random() < 0.3:
                        train_data.append(new_line)

    print(c, c1, c2)
    for k,v in my_item2vid2.items():
        my_item2vid[k] = v
    ...
    if args.do_test or args.do_eval:
        ...
        torch.save(test_result, os.path.join(args.output_dir, 'predictions-'+args.eval_split+'.pth'))
    if args.do_eval:

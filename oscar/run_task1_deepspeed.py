...
class RetrievalDataset(Dataset):
    ...
    def get_image_related(self, img_key, cap_id=None):
        ...
        if not self.is_train:
            ...
            # all_concepts = sorted(concept_counter, key=lambda x: -concept_counter[x])
            all_concepts = [k[0] for k in concept_counter.most_common()]
        all_caps = self.tokenizer.sep_token.join(all_caps)
        # all_concepts = list(set(all_concepts))
        return all_caps, all_concepts
    ...
    def __getitem__(self, index):
        if self.is_train:
            ...
            img_all_cap, img_all_con = self.get_image_related(img_key, cap_id=cap_idxs[1])
            ...
            if self.args.clip_neg_sampling and random.random() <= self.args.clip_neg_prob:
                if random.random() <= 0.5:
                    ...
                    neg_all_cap, neg_all_con = self.get_image_related(neg_img_key, cap_id=-1)
                ...
                if random.random() <= 0.5:
                    ...
                    neg_all_cap, neg_all_con = self.get_image_related(self.img_keys[img_idx_neg], cap_id=-1)
                    example_neg = self.tensorize_example(caption, feature_neg, text_b=od_labels_neg, text_c=extra_concept_pos, ex_text=neg_all_cap, ex_con=neg_all_con)
            ...
            # img_key = self.img_keys[img_idx]
            feature = self.get_image(img_key)
            img_all_cap, img_all_con = self.get_image_related(img_key, cap_id=None)
            caption = self.captions[cap_idxs[0]][cap_idxs[1]]
            od_labels = self.get_od_labels(img_key)
            if self.args.extra_concept:
        ...
        for step, (in_index, batch) in enumerate(train_dataloader):
            ...
            if isinstance(batch[1], tuple) or isinstance(batch[1], list):
                ...
                for i in range(len(batch[1])):
                    inputs['attention_mask'].append(torch.cat((batch[1][i], batch[6][i]), dim=0))
            else:
                batch = tuple(t.to(model_engine.device) for t in batch)
                inputs = {
                    'input_ids':      torch.cat((batch[0], batch[5]), dim=0),
...
def main():
    ...
    parser.add_argument('--neg_con_detect', action='store_true', help='whether to add a token level concept detection task')
    parser.add_argument('--iter_pf', action='store_true', help='if to iterate all possible pre-factors')
    ...
    if args.do_test or args.do_eval:
        ...
        if args.do_eval:
            if args.iter_pf:
                ...
                best_prf = None
                for pf in range(10):
                    eval_result = evaluate(test_dataset, test_result, pr_factor=0.1*pf)
                    rank_accs = eval_result['i2t_retrieval']
                    c_rsum = rank_accs['R@1']+rank_accs['R@5']+rank_accs['R@10']
                    if test_dataset2:
                        t2i_result = evaluate(test_dataset2, t2i_result, pr_factor=0.1*pf)
                    ...
                    if c_rsum > best_rsum:
                        best_rsum = c_rsum
                        best_prf = pf*0.1
                logger.info("best rsum got %s with pre-factor %s" % (best_rsum*100, best_prf))
            else:

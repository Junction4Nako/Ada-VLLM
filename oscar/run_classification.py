...
class ClassificationDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, data_in, is_train=True):
        """
...
def main():
    ...
    test_bili = Counter([line[1] for line in val_data])
    dev_bili = Counter([line[1] for line in dev_data]) if len(dev_data)>0 else None
    print("training ratio:", train_bili.most_common())
    print("test retio:", test_bili.most_common())
    if dev_bili is not None:
        print("dev ration", dev_bili.most_common())
    if args.do_train:
        train_dataset = ClassificationDataset(tokenizer, args, train_data, is_train=True)
        if args.evaluate_during_training:
            val_dataset = ClassificationDataset(tokenizer, args, val_data, is_train=False)
        else:
            val_dataset = None
    ...
    if args.do_test or args.do_eval:
        ...
        test_dataset = ClassificationDataset(tokenizer, args, val_data, is_train=False)
        checkpoint = args.eval_model_dir
        assert op.isdir(checkpoint)
        logger.info("Evaluate the following checkpoint: %s", checkpoint)
        model = model_class.from_pretrained(checkpoint, config=config)

...
class RetrievalDataset(Dataset):
    ...
    def tensorize_example(self, text_a, img_feat, text_b=None, 
        ...
        tokens_a = [self.tokenizer.cls_token] + tokens_a + [self.tokenizer.sep_token]
        segment_ids_a = [cls_token_segment_id] + [sequence_a_segment_id] * (len(tokens_a) + 1)
        seq_a_len = len(tokens_a)
        seq_padding_len_a = self.max_seq_len - seq_a_len
        tokens_a += [self.tokenizer.pad_token] * seq_padding_len_a
        segment_ids_a += [pad_token_segment_id] * seq_padding_len_a
        input_ids_a = self.tokenizer.convert_tokens_to_ids(tokens_a)
        if text_b:
            tokens_b = self.tokenizer.tokenize(text_b)
            if len(tokens_b) > self.args.max_od_length - 1:
                tokens_b = tokens_b[: (self.args.max_od_len - 1)]
            tokens_b += [self.tokenizer.sep_token]
            segment_ids_b = [sequence_b_segment_id] * len(tokens_b)
            seq_b_len = len(tokens_b)
            seq_padding_len_b = self.args.max_od_length - seq_b_len
        ...
        if att_mask_type == "CLR":
            attention_mask_t = [1] * seq_a_len + [0] * seq_padding_len_a
            attention_mask_i = [1] * seq_b_len + [0] * seq_padding_len_b + \
        ...
        input_ids_b = None if input_ids_b is None else torch.tensor(input_ids_b, dtype=torch.long)
        attention_mask_t = torch.tensor(attention_mask_t, dtype=torch.long)
        attention_mask_i = torch.tensor(attention_mask_i, dtype=torch.long)
        segment_ids_a = torch.tensor(segment_ids_a, dtype=torch.long)
        segment_ids_b = None if input_ids_b is None else torch.tensor(segment_ids_b, dtype=torch.long)
        return (input_ids_a, input_ids_b, attention_mask_t, attention_mask_i, segment_ids_a, segment_ids_b, img_feat)
    ...
    def __getitem__(self, index):
        if self.is_train:
            ...
            example_pair = tuple(list(example) + [1])
...
def main():
    ...
    parser.add_argument("--max_od_length", default=40, type=int, help='The maximum total sequence length of od tags')

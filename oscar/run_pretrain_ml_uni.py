...
from oscar.modeling.modeling_vlbert_pretrain import BiBertImgForPreTrainingUni
...
MODEL_CLASSES = {
    'bert': (BertConfig, BiBertImgForPreTrainingUni, BertTokenizer),
}
...
def main():
    ...
    if args.visual_learning:
        ...
        model_class = BiBertImgForPreTrainingUni
    load_num = 0
    ...
    for step, (batch, batch_extra) in enumerate(zip(train_dataloader, train_dataloader_extra), start_iter):
        ...
        def forward_backward(image_features, input_ids_a, input_mask_a, segment_ids_a,lm_label_ids_a, input_ids_b, input_mask_b, 
            ...
            if args.deepspeed:
                ...
                outputs, debug_out = model_engine(input_ids_a=input_ids_a, token_type_ids_a=segment_ids_a, attention_mask_a=input_mask_a,
                                input_ids_b=input_ids_b, img_feats=image_features, use_hungarian=args.use_hungarian, phrase_layer=args.wra_layer,
            ...
            if (step + 1) % args.ckpt_period == 0:
                if get_rank() == 0:
                    torch.save(outputs, '/opt/tiger/debug/uni/test_output.pt')
                    torch.save({'input_ids_a':input_ids_a, 'token_type_ids_a':segment_ids_a, 'attention_mask_a':input_mask_a, \
                                    ...
                                    'token_type_ids_b':segment_ids_b, 'attention_mask_b':input_mask_b, 'masked_lm_labels_b':lm_label_ids_b, 'phrase_mask_label':phrase_mask_label,
                                    'phrase_index':phrase_index, 'img_index':image_index, 'max_tag_length':args.max_tag_length, 'phrase_mod':args.wra_mod, 'qa_is_next':qa_is_next, 'phrase_layer':args.wra_layer}, '/opt/tiger/debug/uni/test_input.pt')
                    torch.save(debug_out, '/opt/tiger/debug/uni/vis_out.pt')
                    model_engine.module.save_pretrained('/opt/tiger/debug/uni/')
            
            total_loss, vis_mlm_loss, retrieval_loss, masked_phrase_loss = outputs
            ...
            return loss.item(), input_ids_a.size(0), vis_mlm_loss, retrieval_loss, masked_phrase_loss
        ...
        if args.visual_learning:
            ...
            loss1, nb_tr_example1, vis_mlm_loss1, retrieval_loss1, phrase_mask_loss1 = forward_backward(
        ...
        phrase_mask_loss2, mlm_loss2 = 0.0, 0
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            metrics_to_log = {
                ...
                'batch_metrics': {'loss': loss1+loss2, 'vis_mlm_loss': vis_mlm_loss1+vis_mlm_loss2, 'CLIP_loss': retrieval_loss1+retrieval_loss2, \
                     'Phrase_mask_loss': phrase_mask_loss1+phrase_mask_loss2}
            }

...
from oscar.modeling.modeling_bert import ImageBertForSequenceClassification_MLM
...
class ClassificationDataset(Dataset):
    ...
    def get_od_labels(self, img_key, new_or_not):
        if new_or_not:
        ...
        if self.args.add_od_labels:
            if new_or_not:
                od_tags = self.add_od_reader.read_many([img_key])[0]
            else:
    ...
    def get_full_tags(self, img_key, new_or_not):
        if img_key.endswith('.jpg'):
            ii = img_key[:-4]
        else:
            ii = img_key
        ...
        if new_or_not:
            return_values = self.add_od_reader.read_many(target_keys)
        else:
    ...
    def tensorize_example(self, text_a, img_feat, text_b=None, text_c=None,
        ...
        if self.is_train:
            for i, tk in enumerate(tokens):
                if tk not in self.tokenizer.special_tokens_map.values():
                    prob = random.random()
                    if prob <= self.args.random_mask:
                        prob /= self.args.random_mask
                        if prob < 0.8:
                            tokens[i] = '[MASK]'
                        elif prob < 0.9:
                            tokens[i] = random.choice(list(self.tokenizer.vocab.items()))[0]
                        mlm_lm_labels.append(self.tokenizer.vocab[tk])
                    else:
                        mlm_lm_labels.append(-1)
        ...
        mlm_lm_labels.extend([-1]*(len(input_ids)-len(mlm_lm_labels)))
        # image features
        ...
        mlm_lm_labels.extend([-1]*self.max_img_seq_len)
        ...
        mlm_lm_labels = torch.tensor(mlm_lm_labels, dtype=torch.long)
        return (input_ids, attention_mask, segment_ids, img_feat, mlm_lm_labels)

    def __getitem__(self, index):
        c_data = self.data_in[index]
        new_or_not = c_data[-1]
        img_key = c_data[0]+'.jpg'
        img_feat = self.get_image(img_key, new_or_not)
        od_tags = self.get_od_labels(img_key, new_or_not)
        all_tags = self.get_full_tags(img_key, new_or_not)
        od_tags = od_tags + ' ' + all_tags
        frame_feat = self.get_frame_feat(img_key, new_or_not)
    ...
    def get_image(self, image_id, new_or_not):
        if new_or_not:
            ...
            vid = self.i2v[ii]
            image_id = vid+'.jpg'
            t_features = self.add_feat_reader.read_many([image_id])[0]
        else:
    ...
    def get_frame_feat(self, image_id, new_or_not):
        if image_id.endswith('.jpg'):
            ii = image_id[:-4]
        else:
            ii = image_id
        ...
        target_keys = [vid+'_'+str(i)+'.jpg' for i in range(self.args.num_frames)]
        if new_or_not:
            return_values = self.add_feat_reader.read_many(target_keys)
        else:
...
def worker_init_fn(_):
    ...
    dataset.add_od_reader = KVReader(dataset.add_od, dataset.num_readers)
    dataset.add_feat_reader = KVReader(dataset.add_feat, dataset.num_readers)
...
def main():
    ...
    parser.add_argument('--eval_split', type=str, default='test', help='which split to evaluarte on')
    parser.add_argument('--extra_data', type=str, default=None, help='extra dataset')
    parser.add_argument('--add_i2v', type=str)
    parser.add_argument('--add_od', type=str)
    parser.add_argument('--add_feat', type=str)
    parser.add_argument('--add_item2tag', type=str)
    ...
    model_class = ImageBertForSequenceClassification_MLM
    ...
    if args.ex_con:
        ...
        with open(args.add_item2tag, 'r') as rf:
            add_item2tag = json.load(rf)
    ...
    with open(args.add_i2v, 'r') as rf:
        add_item2vid = json.load(rf)
        # for k,v in add_item2vid.items():
        #     if k not in my_item2vid:
        #         my_item2vid[k] = v[2:]
    ...
    for line in data_csv:
        if line[0] not in test_ids:
            if line[0] not in hb_data and line[0] not in good_ids:
                continue
        if line[0] not in my_item2vid:
            continue
        if line[0] in add_item2vid and line[0] not in test_ids:
            continue
        old_set.add(line[0])
    ...
    add_data = open(args.extra_data, 'r')
    add_csv = csv.reader(add_data)
    next(add_csv)
    add_safe_data = json.load(open(os.path.split(args.add_i2v)[0]+'/add_i2v_V3.json', 'r'))
    for line in add_csv:
        if line[0] in test_ids:
            continue
        if line[0] not in add_item2vid:
            continue
        if line[0] in old_set:
            continue
        tmp_v = add_item2vid[line[0]]
        if len(tmp_v) == 32 and line[0] not in add_safe_data:
            my_item2vid[line[0]] = tmp_v[2:]
        else:
            my_item2vid[line[0]] = tmp_v
        new_line = [info for info in line]
        new_line[1] = int(new_line[1])
        if args.ex_con:
            tmp = [tag2id[i] for i in add_item2tag[line[0]].split(';') if i in tag2id]
            # print(tmp)
            new_line.append(tmp)
        new_line.append(1)
        train_data.append(new_line)

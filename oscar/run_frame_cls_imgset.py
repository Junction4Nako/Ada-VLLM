...
from oscar.modeling.modeling_bert2 import ImageBertForSequenceClassificationR
...
class ClassificationDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, data_in, is_train=True, i2v=None, i2n=None):
        """
    ...
    def get_full_tags(self, img_key):
        ...
        vid = self.i2v[ii]
        c_n = self.i2n[ii]
        f_n = min(c_n-1, self.args.num_frames)
        target_keys = [vid+'_'+str(i+1)+'.jpg' for i in range(f_n)]
    ...
    def tensorize_example(self, text_a, img_feat, text_b=None, text_c=None, text_d=None,
        ...
        if text_b:
            ...
            if len(tokens_b) > self.args.max_seq_length - len(tokens) - 1:
                ...
                assert(num_extra_tokens >= 0)
                tokens_b = tokens_b[: num_left_tokens]
            tokens += tokens_b
            # tokens += tokens_b + [self.tokenizer.sep_token]
            segment_ids += [sequence_b_segment_id] * len(tokens_b)
        if text_d:
            tokens_d = self.tokenizer.tokenize(text_d)
            if len(tokens_d) > self.max_seq_len - len(tokens) - 1:
                num_left_tokens = max(0, self.max_seq_len - len(tokens) - 1) # to avoid -1
                assert(num_extra_tokens >= 0)
                tokens_d = tokens_d[: num_left_tokens]
            tokens += tokens_d
            segment_ids += [sequence_b_segment_id] * len(tokens_d)
        ...
        # print(input_ids.shape, attention_mask.shape, segment_ids.shape)
    ...
    def __getitem__(self, index):
        ...
        caption = c_data[2]
        label = c_data[1]
        r_labels = [i-1 for i in c_data[-1]]
        tmp_label = torch.zeros(self.args.num_reasons)
        tmp_label[r_labels] = 1
        if self.args.ex_con:
            text_c = c_data[-2]
        else:
            text_c = None
        example = self.tensorize_example(caption, img_feat, text_b=od_tags, text_c=text_c, text_d = all_tags)
        return index, tuple(list(example) + [label, tmp_label])
    ...
    def get_frame_feat(self, image_id):
        ...
        f_n = min(c_n-1, self.args.num_frames)
        if f_n == 0:
            return torch.zeros([0, self.args.img_feat_dim])
        target_keys = [vid+'_'+str(i+1)+'.jpg' for i in range(f_n)]
...
def compute_score_with_logits2(logits, labels):
    logit_ = torch.sigmoid(logits)
    preds = (logit_ > 0.5).to(labels.dtype)
    scores = (preds == labels).float().mean()
    return scores
...
def train(args, train_dataset, val_dataset, model, tokenizer):
    ...
    for epoch in range(int(args.num_train_epochs)):
        for step, (_, batch) in enumerate(train_dataloader):
            ...
            if args.loss_weight:
                p_weights, r_weights = get_loss_weight(args)
                inputs['loss_weights'] = p_weights.to(args.device)
                inputs['r_weights'] = r_weights.to(args.device)
            inputs = prepare_inputs(inputs, args)
            outputs = model(**inputs)
            loss, logits, r_logits = outputs[:3]
            if args.n_gpu > 1: 
            ...
            batch_score_r = compute_score_with_logits2(r_logits, inputs['r_labels']).item()
            global_acc_r += batch_score_r
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.logging_steps == 0:
                    logger.info("Epoch: {}, global_step: {}, lr: {:.6f}, loss: {:.4f} ({:.4f}), " \
                        "score: {:.4f} ({:.4f}), reason score {:.4f} ({:.4f})".format(epoch, global_step, 
                        optimizer.param_groups[0]["lr"], loss, global_loss / global_step, 
                        batch_acc, global_acc / global_step, batch_score_r, global_acc_r/global_step)
                    )
...
def test(args, model, eval_dataset):
    ...
    for indexs, batch in tqdm(eval_dataloader):
        ...
        with torch.no_grad():
            ...
            logits, r_logits = model(**inputs)[:2]
...
def get_loss_weight(args):
    ...
    elif args.pos_neg == 10:
        raw_weights = torch.FloatTensor([0.2, 2])
    elif args.pos_neg == 5:
        raw_weights = torch.FloatTensor([0.4, 2])
    elif args.pos_neg == 4:
        raw_weights = torch.FloatTensor([0.5, 2])
    else:
        raw_weights = torch.FloatTensor([1, 1])
    all_weights = torch.cat([raw_weights]*args.n_gpu, dim=0)
    r_weights = torch.FloatTensor(args.pos_weight)
    return all_weights, torch.cat([r_weights]*args.n_gpu, dim=0)
...
def main():
    ...
    parser.add_argument('--only_dressup', action='store_true')
    parser.add_argument('--pos_neg', type=int, default=1)
    parser.add_argument('--visual_random', action='store_true')
    parser.add_argument('--item2reason', type=str, default=None)
    parser.add_argument('--r_lambda', type=float, default=1)
    parser.add_argument('--reason_penalty', action='store_true')
    parser.add_argument('--export_jit', action='store_true')
    parser.add_argument('--item2num', type=str, default=None)
    ...
    i2n = json.load(open(args.item2num, 'r'))

    item2reason = [line for line in csv.reader(open(args.item2reason, 'r'))]
    i2r = {}
    num_reasons = 0
    for line in item2reason[1:]:
        if line[2] == 'NULL':
            i2r[line[0]] = []
        else:
            try:
                tmp = [int(i) for i in line[2].split(',')]
            except:
                print(line)
                raise ValueError
            i2r[line[0]] = tmp
            num_reasons = max(num_reasons, max(tmp))

    del(item2reason)
    args.num_reasons = num_reasons
    ...
    model_class = ImageBertForSequenceClassificationR
    ...
    if args.do_train:
        ...
        config.r_lambda = args.r_lambda
        config.num_reasons = num_reasons
        config.reason_penalty = args.reason_penalty
        if args.ex_con:
            if not args.refined:
        ...
        if args.export_jit:
            ...
            model.save(os.path.join(args.output_dir, 'model_jit.pt'))
            # return None
        if args.half_evaluation:
            model = model.half()
            args.dtype = torch.float16
    ...
    next(data_csv)
    r_bili = {i:[0,0] for i in range(num_reasons)}
    for k,v in i2r.items():
        ...
        for i in range(num_reasons):
            if i+1 in v:
                ...
                r_bili[i][0] += 1
    print(r_bili)
    args.pos_weight = [min(10,r_bili[i][0]/(r_bili[i][1]+0.0001)) for i in range(num_reasons)]
    for line in data_csv:
        if line[0] not in test_ids:
            if hb_data is not None:
                if line[0] not in hb_data:
        ...
        if line[0] not in my_item2vid:
            continue
        if args.only_dressup:
            if not len(item2tag[line[0]].split(';')) == 1:
                if item2tag[line[0]].split(';')[1]!='穿搭':
                    continue
        ...
        if args.ex_con:
            ...
            # print(tmp)
            new_line.append(tmp)
        if line[0] not in i2r:
            continue
        new_line.append(i2r[line[0]])
    ...
    c1 = Counter([line[1] for line in train_data])
    c2 = Counter([line[1] for line in val_data])
    tag1 = Counter([item2tag[line[0]].split(';')[1] for line in train_data if len(item2tag[line[0]].split(';'))>1])
    tag2 = Counter([item2tag[line[0]].split(';')[1] for line in val_data if len(item2tag[line[0]].split(';'))>1])
    print('training:', c1.most_common())
    print(tag1.most_common())
    print('test:', c2.most_common())
    print(tag2.most_common())
    if len(dev_data) > 0:
        c3 = Counter([line[1] for line in dev_data])
        print('dev:', c3.most_common())
    if args.do_train:
    ...
    if args.do_test or args.do_eval:
        ...
        if args.eval_split == 'test':
            ...
            vd = train_data
        test_dataset = ClassificationDataset(tokenizer, args, vd, is_train=False, i2v=my_item2vid, i2n=i2n)
        checkpoint = args.eval_model_dir
        assert op.isdir(checkpoint)
        logger.info("Evaluate the following checkpoint: %s", checkpoint)
        # model = model_class.from_pretrained(checkpoint, config=config)

...
from transformers_past.pytorch_transformers.modeling_bert import (BertEmbeddings, 
...
class BiBertImgForPreTraining2(ImgPreTrainedModel): # a version with weakly-supervised grounding
    ...
    def forward(self, input_ids_a, token_type_ids_a=None, attention_mask_a=None, masked_lm_labels_a=None, qa_is_next=None, use_hungarian=False,
            input_ids_b=None, token_type_ids_b=None, attention_mask_b=None, masked_lm_labels_b=None, max_tag_length=20, phrase_layer=None, phrase_mask_label=None,
        ...
        if phrase_mask_label is not None:
            ...
            if not torch.isnan(mask_phrase_loss):
                total_loss += mask_phrase_loss # avoid no masking
...
class BiBertImgForCVLM(ImgPreTrainedModel): # a version with weakly-supervised grounding
    r"""
        Multi-modal stage for pretraining BiImageBERT
        **masked_lm_labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size, sequence_length)``:
            Labels for computing the masked language modeling loss.
    ...
    def __init__(self, config):
        super(BiBertImgForCVLM, self).__init__(config)

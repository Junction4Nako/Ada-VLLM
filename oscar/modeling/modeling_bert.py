    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, soft_label=False,
        ...
        if labels is not None:
            if self.num_labels == 1: #  doing regression
                ...
                elif self.loss_type == 'focal': # focal loss
                    loss_fct = FocalLoss(gamma=self.focal_gamma)
...
class ImageBertForSequenceClassification_ML(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if hasattr(config, 'classifier'):
            ...
            if config.classifier == 'linear':
                self.my_classifier = nn.Linear(config.hidden_size,
                                            self.config.num_labels)
            elif config.classifier == 'mlp':
            ...
            self.my_classifier = nn.Linear(config.hidden_size, self.config.num_labels)  # original
        self.apply(self.init_weights)
    ...
    def reinit_cls_head(self):
        # make a re-initialization for the classifier
        self.my_classifier.apply(self.init_weights)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, soft_label=False,
        ...
        pooled_output = self.dropout(pooled_output)
        logits = self.my_classifier(pooled_output)
...
class ImageBertForSequenceClassificationR(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids:Optional[torch.Tensor]=None, attention_mask:Optional[torch.Tensor]=None, labels:Optional[torch.Tensor]=None, soft_label=False, r_lamba=None,
            position_ids:Optional[torch.Tensor]=None, head_mask:Optional[torch.Tensor]=None, img_feats:Optional[torch.Tensor]=None, concep_span=None, loss_weights=None, r_labels=None, r_weights=None):
        outputs = self.bert(input_ids, position_ids=position_ids, token_type_ids=token_type_ids,
                            attention_mask=attention_mask, head_mask=head_mask, img_feats=img_feats)
        pooled_output = outputs[1]
        if concep_span is not None:
...
class ImageBertEmbedding(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(ImageBertEmbedding, self).__init__(config)
        self.config = config
        ...
        if hasattr(config, 'target_dim'):
            self.mapping = nn.Linear(config.hidden_size,
    ...
    def forward(self, input_ids, token_type_ids:Optional[torch.Tensor]=None, attention_mask:Optional[torch.Tensor]=None,
        ...
        outputs = self.bert(input_ids, position_ids=position_ids, token_type_ids=token_type_ids,
                            attention_mask=attention_mask, head_mask=head_mask, img_feats=img_feats)
        # print(outputs[0].shape, attention_mask.shape)
        if self.emb_type == 'CLS':
            pooled_output = outputs[1]
        else:
            sequence_output = outputs[0]
            sequence_output = sequence_output * attention_mask.unsqueeze(-1)
            pooled_output = sequence_output.sum(dim=1)
            pooled_output = pooled_output / (torch.sum(attention_mask, dim=1).unsqueeze(-1))
        if self.mapping is not None:
            pooled_output = self.dropout(pooled_output)
            pooled_output = self.mapping(pooled_output)

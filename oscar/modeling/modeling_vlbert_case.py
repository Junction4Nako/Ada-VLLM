...
class BertImgModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        outputs = (sequence_output[0], pooled_output,) + encoder_outputs[1:]
        return outputs
...
class BiBertImgModel2(BertPreTrainedModel):
    ...
    def forward(self, input_ids_a, token_type_ids_a=None, attention_mask_a=None, max_tag_length=None,
        ...
        tmp_txt_encoder_outputs = self.txt_encoder(embedding_output_a, extended_attention_mask_a,
                        head_mask=head_mask, encoder_history_states=encoder_history_states)[0]
        txt_encoder_outputs = tmp_txt_encoder_outputs[0]
        txt_attention_map = tmp_txt_encoder_outputs[-1]

        tmp_vis_encoder_outputs = self.vis_encoder(embedding_output_b, extended_attention_mask_b,
                        head_mask=head_mask, encoder_history_states=encoder_history_states)[0]
        vis_encoder_outputs = tmp_vis_encoder_outputs[0]
        vis_attention_map = tmp_vis_encoder_outputs[-1]
        ...
        encoder_outputs, mid_joint = self.mul_encoder(joint_bi_output,
                ...
                encoder_history_states=encoder_history_states)
        sequence_output = encoder_outputs[0]
        mul_attention_map = encoder_outputs[-1]
        ...
        return outputs, single_stream_output, hard_indexes, mid_output, (txt_attention_map, vis_attention_map, mul_attention_map)
    ...
    def forward_single(self, input_ids_a, token_type_ids_a=None, attention_mask_a=None, max_tag_length=None,
        ...
        return single_stream_output, (txt_encoder_outputs, vis_encoder_outputs)

    def forward_hard(self, input_ids_a, token_type_ids_a=None, attention_mask_a=None, max_tag_length=None,
            position_ids_a=None, input_ids_b=None, token_type_ids_b=None, attention_mask_b=None, phrase_layer=None,
            position_ids_b=None, head_mask=None, img_feats=None, encoder_history_states=None, encode_hn=False):
        if attention_mask_a is None:
            attention_mask_a = torch.ones_like(input_ids_a)
        ...
        if encode_hn:
            ...
            # hard_img_index_full = torch.cat([torch.index_select(hard_img_index, dim=0, index=dice_index[:(n_sample//2)]), 
            #                     torch.index_select(torch.arange(n_sample, device=hard_img_seq.device), dim=0, index=dice_index[(n_sample//2):])], dim=0)
            hard_img_index_full = hard_img_index
            hard_txt_index_full = torch.arange(n_sample, device=hard_img_seq.device)
            
            # encoding for hard-negatives
            hard_encoder_outputs, mid_hard = self.mul_encoder(hard_img_seq, hard_img_mask, head_mask=head_mask,
                            encoder_history_states=encoder_history_states, return_at_layer=phrase_layer)
            hard_encoder_outputs = hard_encoder_outputs[0]
            hard_pooled_output = self.pooler(hard_encoder_outputs)
        else:
...
class BiBertImgModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids_a, token_type_ids_a=None, attention_mask_a=None, max_tag_length=None,
        ...
        outputs = (sequence_output, pooled_output,) + (hard_encoder_outputs, hard_pooled_output)
        single_stream_output = (txt_encoder_outputs, vis_encoder_outputs, sim_mat)
        return outputs, single_stream_output, (txt_attention_map, vis_attention_map, mul_attention_map)
...
class BertLMPhraseHead(nn.Module):
    def __init__(self, config):
        super(BertLMPhraseHead, self).__init__()
        self.transform = BertPredictionHeadTransform(config)

        self.decoder = nn.Linear(config.hidden_size,
                                config.phrase_vocab_size,
                                bias=False)

        self.bias = nn.Parameter(torch.zeros(config.phrase_vocab_size))
...
class BiBertImgForPreTraining2(ImgPreTrainedModel): # a version with weakly-supervised grounding
    ...
    def forward(self, input_ids_a, token_type_ids_a=None, attention_mask_a=None, masked_lm_labels_a=None, qa_is_next=None, use_hungarian=False,
        ...
        # visual encoder MLM
        vis_lm_mask = masked_lm_labels_b > -1
        nums_per_sample = torch.sum(vis_lm_mask, dim=1)
        vis_masked_seq_output = torch.masked_select(vis_encoder_outputs, vis_lm_mask.unsqueeze(-1)).reshape(-1, self.config.hidden_size)
        vis_lm_score = self.half_mlm(vis_masked_seq_output)
        masked_vis_lm_labels = torch.masked_select(masked_lm_labels_b, vis_lm_mask).reshape(-1)
        if use_hungarian:
            # use hungarian loss
            masked_vis_lm_labels = HungarianMatch(vis_lm_score, masked_vis_lm_labels, nums_per_sample)
        vis_mlm_loss = ce_loss(vis_lm_score, masked_vis_lm_labels)
...
class BiBertImgForPreTrainingUni(ImgPreTrainedModel): # a version with weakly-supervised grounding
    r"""
        Uni-modal training model
        **masked_lm_labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size, sequence_length)``:
            Labels for computing the masked language modeling loss.
            Indices should be in ``[-1, 0, ..., config.vocab_size]`` (see ``input_ids`` docstring)
            Tokens with indices set to ``-1`` are ignored (masked), the loss is only computed for the tokens with labels
    ...
    def __init__(self, config):
        super(BiBertImgForPreTrainingUni, self).__init__(config)

        #self.bert = BertModel(config) # original BERT
        self.bert = BiBertImgModel2(config)
        self.cls = BertPreTrainingHeads(config, only_vocab=True)
        self.half_mlm = BertLMPredictionHead(config, only_vocab=True)
        self.phrase_mlm = BertLMPhraseHead(config)
        # self.qa_head = nn.Linear(config.hidden_size, config.qa_answer_size)
        self.only_vocab_size = config.only_word_size
        self.qa_seq_relation = nn.Linear(config.hidden_size, 2)
        self.num_seq_relations = config.num_contrast_classes if hasattr(config, "num_contrast_classes") else 2
    ...
    def forward(self, input_ids_a, token_type_ids_a=None, attention_mask_a=None, use_hungarian=False,
            input_ids_b=None, token_type_ids_b=None, attention_mask_b=None, masked_lm_labels_b=None, phrase_layer=None,
            position_ids_a=None, position_ids_b=None, head_mask=None, img_feats=None, phrase_mask_label=None):
        global_output, seq_output = self.bert.forward_single(input_ids_a=input_ids_a, position_ids_a=position_ids_a, token_type_ids_a=token_type_ids_a,
                            attention_mask_a=attention_mask_a, head_mask=head_mask, img_feats=img_feats, input_ids_b=input_ids_b, 
                            position_ids_b=position_ids_b, token_type_ids_b=token_type_ids_b, attention_mask_b=attention_mask_b)

        global_txt, global_img = global_output
        sim_mat = global_txt @ global_img.t()
        txt_encoder_outputs, vis_encoder_outputs = seq_output
        # txt_encoder_outputs, vis_encoder_outputs, sim_mat = single_stream_output
        ...
        if use_hungarian:
            ...
            masked_vis_lm_labels = HungarianMatch(vis_lm_score, masked_vis_lm_labels, nums_per_sample)
        vis_mlm_loss = ce_loss(vis_lm_score, masked_vis_lm_labels)
        test_out = (vis_encoder_outputs, vis_lm_score, masked_vis_lm_labels)
        ...
        total_loss = vis_mlm_loss+retrieval_loss
        outputs = (vis_mlm_loss, retrieval_loss)
        ## phrase-level concept modeling
        if phrase_mask_label is not None:
            phr_lm_mask = phrase_mask_label > -1
            nums_per_sent = torch.sum(phr_lm_mask, dim=1)
            phr_masked_seq_output = torch.masked_select(txt_encoder_outputs, phr_lm_mask.unsqueeze(-1)).reshape(-1, self.config.hidden_size)
            phr_lm_score = self.phrase_mlm(phr_masked_seq_output)
            masked_phr_lm_labels = torch.masked_select(phrase_mask_label, phr_lm_mask).reshape(-1)
            if use_hungarian:
                # use hungarian loss
                masked_phr_lm_labels = HungarianMatch(phr_lm_score, masked_phr_lm_labels, nums_per_sent)
            mask_phrase_loss = ce_loss(phr_lm_score, masked_phr_lm_labels)
            total_loss += mask_phrase_loss
            outputs += (mask_phrase_loss,)
        ...
        return outputs, test_out  # (loss), prediction_scores, seq_relationship_score, (hidden_states), (attentions)


class BiBertImgForPreTrainingMul(ImgPreTrainedModel): # a version with weakly-supervised grounding
    r"""
        Multi-modal stage for pretraining BiImageBERT
        **masked_lm_labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size, sequence_length)``:
            Labels for computing the masked language modeling loss.
            Indices should be in ``[-1, 0, ..., config.vocab_size]`` (see ``input_ids`` docstring)
            Tokens with indices set to ``-1`` are ignored (masked), the loss is only computed for the tokens with labels
    ...
    def __init__(self, config):
        super(BiBertImgForPreTrainingMul, self).__init__(config)
    ...
    def tie_weights(self):
        ...
        # as the tie_weights will be called in from_pretrained
        # threfore not needed in 2-staged training!
        # self._tie_or_clone_weights(self.half_mlm.decoder, self.bert.embeddings.word_embeddings,
        #                            only_vocab=True, only_word_size=self.only_vocab_size)

    def forward(self, input_ids_a, token_type_ids_a=None, attention_mask_a=None, masked_lm_labels_a=None, qa_is_next=None, use_hungarian=False,
            input_ids_b=None, token_type_ids_b=None, attention_mask_b=None, masked_lm_labels_b=None, max_tag_length=20, phrase_layer=None,
            position_ids_a=None, position_ids_b=None, head_mask=None, img_feats=None,is_img_match=None, img_index=None, phrase_index=None, phrase_mod='sample', phrase_mask_label=None):
        outputs, single_stream_output, hard_indexes, mid_output = self.bert(input_ids_a=input_ids_a, position_ids_a=position_ids_a, token_type_ids_a=token_type_ids_a,
                            attention_mask_a=attention_mask_a, head_mask=head_mask, img_feats=img_feats, phrase_layer=phrase_layer,
                            input_ids_b=input_ids_b, position_ids_b=position_ids_b, token_type_ids_b=token_type_ids_b,
                            attention_mask_b=attention_mask_b, max_tag_length=max_tag_length, encode_hn=True)
        ...
        if use_hungarian:
            ...
            masked_vis_lm_labels = HungarianMatch(vis_lm_score, masked_vis_lm_labels, nums_per_sample)
        vis_mlm_loss = ce_loss(vis_lm_score, masked_vis_lm_labels)
        debug_outputs = (vis_encoder_outputs, vis_lm_score, masked_vis_lm_labels)
        ...
        return outputs, debug_outputs  # (loss), prediction_scores, seq_relationship_score, (hidden_states), (attentions)



@torch.no_grad()
def HungarianMatch(logits, labels, nums_per_sample):
    """
    logits = [n_queries, num_labels]
    labels = [n_queries]
    """
    probs = 1 - logits.softmax(-1)
    nums_per_sample = nums_per_sample.detach().cpu().numpy().tolist()
    selected_labels = []
    for c_prob, c_label in zip(probs.split(nums_per_sample, 0), labels.split(nums_per_sample, 0)):
        prob_mat = c_prob[:, c_label] # a probability matrix for all possibilities
        indices = linear_sum_assignment(prob_mat.cpu())[1]
        selected_labels.append(c_label[indices])
    return torch.cat(selected_labels)
...
class BiImageBertForRetrieval(BertPreTrainedModel):
    ...
    def forward_emb(self, input_ids_a, token_type_ids_a=None, attention_mask_a=None,
        ...
        global_txt, global_img = self.bert.forward_single(input_ids_a=input_ids_a, position_ids_a=position_ids_a, token_type_ids_a=token_type_ids_a,
                            attention_mask_a=attention_mask_a, head_mask=head_mask, img_feats=img_feats,
                            input_ids_b=input_ids_b, position_ids_b=position_ids_b, token_type_ids_b=token_type_ids_b,
                            attention_mask_b=attention_mask_b, max_tag_length=max_tag_length)[0]
...
class BiImageBertForGrounding(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(BiImageBertForGrounding, self).__init__(config)
        self.num_labels = 1
        # self.loss_type = config.loss_type
        self.config = config
        if config.img_feature_dim > 0:
    ...
    def forward(self, input_ids_a, token_type_ids_a=None, attention_mask_a=None, labels=None, phrase_layer=None,
            input_ids_b=None, token_type_ids_b=None, attention_mask_b=None, max_tag_length=20, mod="mean",
            position_ids_a=None, position_ids_b=None, head_mask=None, img_feats=None, phrase_index=False, loss_mod=None):
        """
        phrase_index: [batch_size, n, max_seq_length]
        """
        if loss_mod is not None:
            encode_hn = True if 'wpg' in loss_mod else False
        else:
            encode_hn = False
        outputs, single_stream_output, hard_indexes, mid_out = self.bert.forward_hard(input_ids_a=input_ids_a, position_ids_a=position_ids_a, token_type_ids_a=token_type_ids_a,
                            attention_mask_a=attention_mask_a, head_mask=head_mask, img_feats=img_feats, phrase_layer=phrase_layer,
                            input_ids_b=input_ids_b, position_ids_b=position_ids_b, token_type_ids_b=token_type_ids_b,
                            attention_mask_b=attention_mask_b, max_tag_length=max_tag_length, encode_hn=encode_hn)
        if phrase_layer is not None:
            sequence_output = mid_out[0]
            hard_sequence_output = mid_out[1]
        else:
            sequence_output = outputs[0]
            hard_sequence_output = outputs[2]

        
        seq_a_length = input_ids_a.shape[1]
        sequence_output = self.dropout(sequence_output)
        vis_sequence_output = sequence_output[:, seq_a_length:]
        txt_sequence_output = sequence_output[:, :seq_a_length]
        cls_output = sequence_output[:, 0]
        num_phrases = torch.sum(phrase_index, dim=-1)
        phrase_mask = num_phrases > 0

        vis_sequence_output = F.normalize(vis_sequence_output, p=2, dim=-1)
        if mod == "mean":
            # [batch_size, n, feature_dim]
            phrase_output = torch.bmm(phrase_index, txt_sequence_output) / (num_phrases.unsqueeze(-1)+1e-8)
            phrase_output = F.normalize(phrase_output, p=2 ,dim=-1)
            # [batch_size, n, n_img]
        elif mod == 'softmax':
            txt_sequence_output = F.normalize(txt_sequence_output, p=2, dim=-1)
            tmp_logits = torch.bmm(txt_sequence_output, vis_sequence_output.permute(0,2,1))
            scale = 1.0 / np.sqrt(txt_sequence_output.size(-1))
            # [batch_size, txt_length, vis_sequence_length]
            vis_attention_mask = attention_mask_b[:, max_tag_length:].to(tmp_logits.dtype)
            # [batch_size, vis_sequence_length]
            vis_mask = (1.0 - vis_attention_mask) * -10000.0
            tmp_logits = tmp_logits.mul_(scale) + vis_mask.unsqueeze(1)
            token_concreteness = F.softmax(tmp_logits, dim=2)
            # [batch_size, txt_length]
            token_concreteness = torch.max(token_concreteness, dim=2).values
            # [batch_size, n, txt_length]
            token_concreteness = token_concreteness.unsqueeze(1).repeat(1, phrase_index.shape[1], 1)
            token_phrase_weight = token_concreteness + (1.0 - phrase_index) * -10000.0
            token_phrase_weight = F.softmax(token_phrase_weight, dim=-1)
            phrase_output = F.normalize(torch.bmm(token_phrase_weight, txt_sequence_output))
        else:
            raise NotImplementedError

        # [batch_size, n, img_seq_length]
        num_regions_per_img = torch.sum(attention_mask_b[:, max_tag_length:], dim=1)
        num_phrases_per_sent = torch.sum(phrase_mask, dim=1)
        logits = torch.bmm(phrase_output, vis_sequence_output.permute(0,2,1))
        # print('phrase output:', phrase_output)
        # print('logits:', logits)
        if loss_mod is None:
            return logits
        elif loss_mod == 'supervised_1':
            ...
            loss = mse(torch.masked_select(labels, label_mask), torch.masked_select(logits, label_mask))
            return (loss, logits)
        elif loss_mod == 'wpg_hard':
            hard_sequence_output = self.dropout(hard_sequence_output)
            hard_vis_sequence_output = hard_sequence_output[:, seq_a_length:]
            hard_txt_sequence_output = sequence_output[:, :seq_a_length]
            num_phrases = torch.sum(phrase_index, dim=-1)
            phrase_mask = num_phrases > 0
            hard_txt_index, hard_img_index = hard_indexes
            hard_attention_mask_b = torch.index_select(attention_mask_b, dim=0, index=hard_img_index)

            hard_vis_sequence_output = F.normalize(hard_vis_sequence_output, p=2, dim=-1)
            if mod == "mean":
                # [batch_size, n, feature_dim]
                hard_phrase_output = torch.bmm(phrase_index, hard_txt_sequence_output) / (num_phrases.unsqueeze(-1)+1e-8)
                hard_phrase_output = F.normalize(hard_phrase_output, p=2 ,dim=-1)
                # [batch_size, n, n_img]
            elif mod == 'softmax':
                hard_txt_sequence_output = F.normalize(hard_txt_sequence_output, p=2, dim=-1)
                hard_tmp_logits = torch.bmm(hard_txt_sequence_output, hard_vis_sequence_output.permute(0,2,1))
                scale = 1.0 / np.sqrt(hard_txt_sequence_output.size(-1))
                # [batch_size, txt_length, vis_sequence_length]
                hard_vis_attention_mask = hard_attention_mask_b[:, max_tag_length:].to(hard_tmp_logits.dtype)
                # [batch_size, vis_sequence_length]
                hard_vis_mask = (1.0 - hard_vis_attention_mask) * -10000.0
                hard_tmp_logits = hard_tmp_logits.mul_(scale) + hard_vis_mask.unsqueeze(1)
                hard_token_concreteness = F.softmax(hard_tmp_logits, dim=2)
                # [batch_size, txt_length]
                hard_token_concreteness = torch.max(hard_token_concreteness, dim=2).values
                # [batch_size, n, txt_length]
                hard_token_concreteness = hard_token_concreteness.unsqueeze(1).repeat(1, phrase_index.shape[1], 1)
                hard_token_phrase_weight = hard_token_concreteness + (1.0 - phrase_index) * -10000.0
                hard_token_phrase_weight = F.softmax(hard_token_phrase_weight, dim=-1)
                hard_phrase_output = F.normalize(torch.bmm(hard_token_phrase_weight, hard_txt_sequence_output))
            else:
                raise NotImplementedError
            hard_logits = torch.bmm(hard_phrase_output, hard_vis_sequence_output.permute(0,2,1))
            hard_num_regions_per_img = num_regions_per_img.index_select(0, index=hard_img_index)

            pos_sims = get_mil_sims(logits=logits, text_index=num_phrases_per_sent, img_index=num_regions_per_img, sim_func=t2i_sim_max)
            neg_sims = get_mil_sims(logits=hard_logits, text_index=num_phrases_per_sent, img_index=hard_num_regions_per_img, sim_func=t2i_sim_max)
            # pos_sims = get_pos_sims(sequence_output=sequence_output_mid, text_index=phrase_index, img_index=img_index)
            # neg_sims = get_pos_sims(sequence_output=hard_sequence_output_mid, text_index=hard_phrase_index, img_index=hard_object_index)
            wra_loss = torch.clamp(neg_sims + 0.2 - pos_sims, min=0)
            wra_valid_mask = (num_phrases_per_sent)>0
            print(pos_sims, neg_sims)
            print(wra_valid_mask)
            wra_loss = torch.mean(torch.masked_select(wra_loss, wra_valid_mask))
            print(wra_loss)
            

        elif loss_mod == 'sample':
            # weakly supervised phrase grouding
            # start_time = time.time()
            valid_phrases = F.normalize(mask_slice_and_stack(sequence_output_mid, phrase_index), p=2, dim=-1)
            valid_images = F.normalize(mask_slice_and_stack(sequence_output_mid, img_index), p=2, dim=-1)
...
def get_mil_sims(logits, text_index, img_index, sim_func):
    # multiple instance learning-based global similarity
    pos_sims = []

    for idx in range(text_index.shape[0]):
        cur_valid_logits = logits[idx]
        cur_valid_txt_index = torch.arange(0, text_index[idx], device=logits.device)
        cur_valid_logits = cur_valid_logits.index_select(0, cur_valid_txt_index)
        cur_valid_img_index = torch.arange(0, img_index[idx], device=logits.device)
        cur_valid_logits =cur_valid_logits.index_select(1, cur_valid_img_index)
        if cur_valid_logits.shape[0] == 0:
            pos_sims.append(torch.zeros(1, dtype=cur_valid_logits.dtype, device=cur_valid_logits.device).squeeze())
        else:
            pos_sims.append(sim_func(cur_valid_logits))

    pos_sims = torch.stack(pos_sims)

    return pos_sims

class ImageBertForGrounding(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(ImageBertForGrounding, self).__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        if config.img_feature_dim > 0:
            self.bert = BertImgModel(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, soft_label=False, phrase_index=None,
            position_ids=None, head_mask=None, img_feats=None, concep_span=None, loss_weights=None, mod=1, loss_mod=None):
        outputs = self.bert(input_ids, position_ids=position_ids, token_type_ids=token_type_ids,
                            attention_mask=attention_mask, head_mask=head_mask, img_feats=img_feats)
        ...
        if mod == "mean":
            # [batch_size, n, feature_dim]
            phrase_output = torch.bmm(phrase_index, txt_sequence_output) / (num_phrases.unsqueeze(-1)+0.0000001)
            phrase_output = F.normalize(phrase_output, p=2 ,dim=-1)
            # [batch_size, n, n_img]
        elif mod == 'softmax':
            txt_sequence_output = F.normalize(txt_sequence_output, p=2, dim=-1)
            ...
            scale = 1.0 / np.sqrt(txt_sequence_output.size(-1))
            # [batch_size, txt_length, vis_sequence_length]
            vis_attention_mask = attention_mask[:, seq_a_length:].to(tmp_logits.dtype)
            # [batch_size, vis_sequence_length]
            vis_mask = (1.0 - vis_attention_mask) * -10000.0
            tmp_logits = tmp_logits.mul_(scale) + vis_mask.unsqueeze(1)
            token_concreteness = F.softmax(tmp_logits, dim=2)
...
class BiBertImgForCase(ImgPreTrainedModel): # a version with weakly-supervised grounding
    ...
    def forward(self, input_ids_a, token_type_ids_a=None, attention_mask_a=None, masked_lm_labels_a=None, qa_ans=None,
            ...
            position_ids_a=None, position_ids_b=None, head_mask=None, img_feats=None,is_img_match=None, img_index=None, phrase_index=None, phrase_mod='sample'):
        logit_scale = self.logit_scale.exp()
        outputs, single_stream_output, hard_indexes, mid_output, attention_maps = self.bert(input_ids_a=input_ids_a, position_ids_a=position_ids_a, token_type_ids_a=token_type_ids_a,
                            attention_mask_a=attention_mask_a, head_mask=head_mask, img_feats=img_feats, phrase_layer=phrase_layer,
                            input_ids_b=input_ids_b, position_ids_b=position_ids_b, token_type_ids_b=token_type_ids_b,
                            attention_mask_b=attention_mask_b, max_tag_length=max_tag_length, encode_hn=False)
        ...
        return outputs, attention_maps  # (loss), prediction_scores, seq_relationship_score, (hidden_states), (attentions)

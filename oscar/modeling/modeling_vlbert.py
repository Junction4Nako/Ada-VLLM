...
class BiImageBertForMultiChoice(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(BiImageBertForMultiChoice, self).__init__(config)
        self.num_labels = config.num_labels
        self.loss_type = config.loss_type
        self.config = config
        if config.img_feature_dim > 0:
        ...
        if hasattr(config, 'classifier'):
            ...
            if config.classifier == 'linear':
                self.classifier = nn.Linear(config.full_hidden_size,
                                            self.config.num_labels)
            elif config.classifier == 'mlp':
                self.classifier = nn.Sequential(
                    nn.Linear(config.full_hidden_size, config.full_hidden_size * config.cls_hidden_scale),
                    nn.ReLU(),
                    nn.Linear(config.full_hidden_size * config.cls_hidden_scale, self.config.num_labels)
                )
        else:
            self.classifier = nn.Linear(config.full_hidden_size, self.config.num_labels)  # original
        self.apply(self.init_weights)
    ...
    def forward(self, input_ids_a, token_type_ids_a=None, attention_mask_a=None, labels=None,
            input_ids_b=None, token_type_ids_b=None, attention_mask_b=None, max_tag_length=20,
            input_ids_b2 = None, token_type_ids_b2=None, attention_mask_b2=None, img_feats2=None, mod='feature',
            position_ids_a=None, position_ids_b=None, position_ids_b2=None, head_mask=None, img_feats=None, soft_label=False):
        if mod == 'feature':
            outputs1, single_stream_output1, hard_indexes1 = self.bert(input_ids_a=input_ids_a, position_ids_a=position_ids_a, token_type_ids_a=token_type_ids_a,
                            attention_mask_a=attention_mask_a, head_mask=head_mask, img_feats=img_feats,
                            input_ids_b=input_ids_b, position_ids_b=position_ids_b, token_type_ids_b=token_type_ids_b,
                            attention_mask_b=attention_mask_b, max_tag_length=max_tag_length, encode_hn=False)
            outputs2, single_stream_output2, hard_indexes2 = self.bert(input_ids_a=input_ids_a, position_ids_a=position_ids_a, token_type_ids_a=token_type_ids_a,
                            attention_mask_a=attention_mask_a, head_mask=head_mask, img_feats=img_feats2,
                            input_ids_b=input_ids_b2, position_ids_b=position_ids_b, token_type_ids_b=token_type_ids_b2,
                            attention_mask_b=attention_mask_b2, max_tag_length=max_tag_length, encode_hn=False)
            sequence_output1, pooled_output1, hard_sequence_output1, hard_pooled_output1 = outputs1
            sequence_output2, pooled_output2, hard_sequence_output2, hard_pooled_output2 = outputs2
            pooled_output = torch.cat([pooled_output1, pooled_output2], dim=-1)
        elif mod == 'sequence':
            outputs = self.bert.forward_joint(input_ids_a=input_ids_a, position_ids_a=position_ids_a, token_type_ids_a=token_type_ids_a,
                            attention_mask_a=attention_mask_a, head_mask=head_mask, img_feats=img_feats, img_feats2=img_feats2,
                            input_ids_b2=input_ids_b2, position_ids_b2=position_ids_b2, token_type_ids_b2=token_type_ids_b2,
                            input_ids_b=input_ids_b, position_ids_b=position_ids_b, token_type_ids_b=token_type_ids_b,
                            attention_mask_b=attention_mask_b, max_tag_length=max_tag_length, attention_mask_b2=attention_mask_b2)
...
class BiImageBertForMultiChoiceT(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(BiImageBertForMultiChoiceT, self).__init__(config)
        self.num_labels = config.num_labels
        self.loss_type = config.loss_type
        self.config = config
        if config.img_feature_dim > 0:
    ...
    def forward(self, input_ids_a1, token_type_ids_a1=None, attention_mask_a1=None, labels=None,
            input_ids_a2=None, token_type_ids_a2=None, attention_mask_a2=None, position_ids_a2=None,
            input_ids_b=None, token_type_ids_b=None, attention_mask_b=None, max_tag_length=20,
            input_ids_b2 = None, token_type_ids_b2=None, attention_mask_b2=None, img_feats2=None, mod='feature',
            position_ids_a1=None, position_ids_b=None, position_ids_b2=None, head_mask=None, img_feats=None, soft_label=False):
        if mod == 'feature':
            outputs1, single_stream_output1, hard_indexes1 = self.bert(input_ids_a=input_ids_a1, position_ids_a=position_ids_a1, token_type_ids_a=token_type_ids_a1,
                            attention_mask_a=attention_mask_a1, head_mask=head_mask, img_feats=img_feats,
                            input_ids_b=input_ids_b, position_ids_b=position_ids_b, token_type_ids_b=token_type_ids_b,
                            attention_mask_b=attention_mask_b, max_tag_length=max_tag_length, encode_hn=False)
            outputs2, single_stream_output2, hard_indexes2 = self.bert(input_ids_a=input_ids_a2, position_ids_a=position_ids_a2, token_type_ids_a=token_type_ids_a2,
                            attention_mask_a=attention_mask_a2, head_mask=head_mask, img_feats=img_feats2,
                            input_ids_b=input_ids_b2, position_ids_b=position_ids_b, token_type_ids_b=token_type_ids_b2,
                            attention_mask_b=attention_mask_b2, max_tag_length=max_tag_length, encode_hn=False)
            sequence_output1, pooled_output1, hard_sequence_output1, hard_pooled_output1 = outputs1
            sequence_output2, pooled_output2, hard_sequence_output2, hard_pooled_output2 = outputs2
...
class BiImageBertForNER(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(BiImageBertForNER, self).__init__(config)
        self.num_labels = config.num_labels
        self.loss_type = config.loss_type
        self.config = config
        if config.img_feature_dim > 0:
    ...
    def forward(self, input_ids_a, token_type_ids_a=None, attention_mask_a=None, labels=None,
        ...
        outputs, single_stream_output, hard_indexes = self.bert(input_ids_a=input_ids_a, position_ids_a=position_ids_a, token_type_ids_a=token_type_ids_a,
                            ...
                            attention_mask_b=attention_mask_b, max_tag_length=max_tag_length, encode_hn=False)
        sequence_output, pooled_output, hard_sequence_output, hard_pooled_output = outputs
        txt_len = input_ids_a.shape[1]
        only_text_output = sequence_output[:,:txt_len,:]

        logits = self.classifier(only_text_output)
        # print('in model batch size', logits.shape)
        if labels is not None:
            loss = -1*self.crf(logits, labels, mask=attention_mask_a.byte(), reduction='mean')
            return loss, logits
        else:
            preds = self.crf.decode(logits, mask=attention_mask_a.byte())
            #print(preds)
            return None, preds

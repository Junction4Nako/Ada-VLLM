...
from .llava_dataset import Llava_Instruct

name2dataset = {
    'llava': Llava_Instruct
}
...
def build_dataset(args, tokenizer):
    ...
    cfg = dict(
        ...
        tokenizer=tokenizer,
        transform=old_transform # image_processor.transform
    )
...
def build_llava_dataset(args):
    ...
    image_processor = registry.get_processor_class(processor_cfg['vis_processor']['train']['name']).from_config(processor_cfg['vis_processor']['train'])
    text_processor = registry.get_processor_class(processor_cfg['text_processor']['train']['name']).from_config(processor_cfg['text_processor']['train'])
    cfg = config['instruct_data']['llava']
    if 'line_sep' in cfg:
        if cfg['line_sep'] == 'line_break':
            cfg['line_sep'] = '\n'
        elif cfg['line_sep'] == 'white_space':
            cfg['line_sep'] = ' '
        else:
            raise ValueError
    cfg['image_transform'] = old_transform
    datasets = [Llava_Instruct(**cfg)]
    # datasets = [ImgOnlyDataset(**cfg)]
    # datasets = [TextOnlyDataset2(input_tsv=args.text_corpus, args=args, seq_len=args.max_seq_length, tokenizer=tokenizer)]
    return datasets
...
def make_data_loader_llava(args, tokenizer, is_distributed=False, arguments=None):
    num_gpus = get_world_size()
    # figure out start iteration
    if arguments is None:
        start_iter = 0
    ...
    # build dataset
    datasets = build_llava_dataset(args)

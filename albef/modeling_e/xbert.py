...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # use to indicate the different pos ids for roberta and bert
        self.is_roberta = config.is_roberta if hasattr(config, 'is_roberta') else False
        print('Is this a roberta model? {}'.format(self.is_roberta))
...
class BertLayer(nn.Module):
    def __init__(self, config, layer_num):
        ...
        # False here for no cross-attention
        self.has_cross_attention = False # (layer_num >= config.fusion_layer)
    ...
    def forward(
        ...
        if self.has_cross_attention and not skip_cross_attention:
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        txt_length = hidden_states.shape[1]
        for i in range(start_layer, output_layer):
            if i == self.config.fusion_layer and not skip_cross_attention:
        ...
        hidden_states = hidden_states[:, :txt_length, :] # truncate the txt part
        if not return_dict:
            return tuple(
                v
                for v in [
...
class BertQAPredictionHead(nn.Module):
    def __init__(self, config, class_num):
        ...
        # The output weights are the same as the input embeddings, but there is
        # an output-only bias for each token.
        self.decoder = nn.Linear(config.hidden_size, class_num, bias=False)
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(
        ...
        masked_sequence_output = torch.masked_select(sequence_output, lm_mask.unsqueeze(-1)).reshape(-1, hidden_size)
        ...
        if labels is not None:
            ...
            masked_lm_loss = loss_fct(prediction_scores, masked_labels)
            # masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), labels.view(-1))
        ...
        return MaskedLMOutput(
            loss=masked_lm_loss,
            logits=prediction_scores,
            # hidden_states=masked_sequence_output,
            hidden_states=outputs.hidden_states,
            attentions=outputs.attentions,
        )
